...
`BDDRoute.wellFormednessConstraints(boolean onlyBgp)` produces the
constraints that distinguish "valid route" from "any bit pattern" (e.g.,
prefix length ≤ 32). Both constraints and assignments must respect this.
...
`ConfigAtomicPredicates` runs once per (configuration, set-of-policies) pair
and is reused for every policy on the device. It contains:
...
`HasSize` only handles the conservative thresholds it can prove.
