...
- **BGP session liveness from VIF telemetry.** The `BgpStatus` field on each VIF's BGP peer (e.g., `up`/`down`) is parsed but not used to gate routing — routes will be modeled as reachable as long as the snapshot lists the association as `associated`, even if the live BGP session is reported down. Real AWS returns `available` associations even when the VIF BGP session is down, which can cause Batfish to model reachability that does not exist operationally.
