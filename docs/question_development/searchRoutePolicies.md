...
4. **Exhaustive testing**: Traditional testing with a few example routes cannot cover the enormous space of possible BGP announcements. Symbolic analysis provides mathematical guarantees.
...
A single `BDDRoute` can represent many concrete routes compactly. For example, a BDD constraint like "prefix is in 10.0.0.0/8 AND has community 65000:100" represents all routes matching those criteria. The space is finite (fixed-width bit fields) but far too large to enumerate.
