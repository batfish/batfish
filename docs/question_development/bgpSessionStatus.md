...
1. **Dynamic local IP inference**: When local IP is not explicitly configured, Batfish infers potential source IPs from the FIB. This may not match the actual IP selection algorithm of all vendors.

2. **No authentication checking**: The questions do not verify that MD5 passwords or other authentication settings match between peers.

3. **Confederation support is complex**: Peers within the same confederation, across confederation boundaries, and external to confederations are all handled, but edge cases may exist (see batfish/batfish#9757).

4. **eBGP single-hop verification**: The 2-hop limit for eBGP single-hop is based on the initiator's configuration. If only the listener is configured as single-hop, the check may not apply correctly.
