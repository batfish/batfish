/**
 * Given the configurations provided, consider all nodes with at least one IPSEC VPN definition:
 * Report whether each such node has an IPSEC VPN connection to each other such node.  
 */
verify {
   $vpn_nodes:set<node>; 
