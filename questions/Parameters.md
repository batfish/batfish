...
* **Regexes:** Regular expressions must be enclosed in `/` like `/abc/`. Batfish uses [Java's syntax and semantics](https://docs.oracle.com/javase/8/docs/api/java/util/regex/Pattern.html#sum) for regular expressions. For simple expressions, this language is similar to others. For example:
...
A specification of flow dispositions, used to identify desired flow actions. Used in questions like [reachability](https://pybatfish.readthedocs.io/en/latest/questions.html#pybatfish.question.bfq.reachability), the
...
There are two coarse-grained flow dispositions: `Success` and `Failure` that indicate that a flow has been successfully delivered, or alternatively, has been dropped somewhere in the network. 
...
* `@connectedTo(ipSpec)` indicates all interfaces with configured IPv4 networks that overlap with IPs denoted with the specifieds IPs (see [`ipSpec`](#interface-specifier-grammar))

* `@interfaceGroup(g, b)` looks in the configured reference library for an interface group with name `g` and book with name `b`.

* `@interfaceType(type1)` indicates all interfaces of type `type1`. The types of interfaces are [listed](#interface-types). 
...
interfaceFunc :=
    ...
    | <b>@interfaceType(</b>interfaceType<b>)</b>
...
* `@addressGroup(g, b)` looks in the configured reference library for an address group name `g` and book name `b`.
...
* `as1border1` specifies the `InterfaceLocation` for *all* interfaces on node `as1border1`. 

* `as1border1[Ethernet0/0]` specifies the `InterfaceLocation` for `Ethernet0/0` on node `as1border1`.

* `@vrf(vrf1)` specifies the `InterfaceLocation` for any interface in `vrf1` on *all* nodes. 
...
* `@role(r, d)` indicates all nodes with role name `r` in dimension name `d`.
