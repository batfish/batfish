...
public class ConvertConfigurationAnswerElement
   ...
   public String prettyPrint() throws JsonProcessingException {
      StringBuilder retString = new StringBuilder("Results from converting vendor configurations\n");
      ...
      for (String name : _warnings.keySet()) {
         ...
         for (Warning warning : _warnings.get(name).getUnimplementedWarnings()) {
            retString.append("    Unimplemented " + warning.getTag() + " : " + warning.getText() + "\n");
         }
      ...
      for (String name : _undefinedReferences.keySet()) {
         ...
         for (String structType : _undefinedReferences.get(name).keySet()) {
            for (String structName : _undefinedReferences.get(name).get(structType)) {
      ...
      for (String name : _unusedStructures.keySet()) {
         ...
         for (String structType : _unusedStructures.get(name).keySet()) {
            for (String structName : _unusedStructures.get(name).get(structType)) {
               retString.append("    " + structType + ": " + structName + "\n");
            }            
         }
      }
