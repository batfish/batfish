...
final class RegexAppAstNode implements AppAstNode {
  private final String _regex;
  private final Pattern _pattern;

  RegexAppAstNode(AstNode regexAst) {
    _regex = ((RegexAstNode) regexAst).getRegex();
    _pattern = ((RegexAstNode) regexAst).getPattern();
  }

  RegexAppAstNode(String regex) {
    _regex = regex;
  ...
  public <T> T accept(AstNodeVisitor<T> visitor) {
    return visitor.visitRegexAppAstNode(this);
  }
  ...
  public boolean equals(Object o) {
    ...
    if (!(o instanceof RegexAppAstNode)) {
      return false;
    }
    RegexAppAstNode that = (RegexAppAstNode) o;
