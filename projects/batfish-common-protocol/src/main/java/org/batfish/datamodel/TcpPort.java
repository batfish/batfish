...
public enum TcpPort {
  ...
  private static final Map<Integer, TcpPort> NUMBER_TO_PORT_MAP = buildNumberToPortMap();

  private static Map<Integer, TcpPort> buildNumberToPortMap() {
    ImmutableMap.Builder<Integer, TcpPort> map = ImmutableMap.builder();
    for (TcpPort protocol : values()) {
      map.put(protocol._number, protocol);
    }
    return map.build();
  }

  public static TcpPort fromNumber(int number) {
    TcpPort ret = NUMBER_TO_PORT_MAP.get(number);
    if (ret == null) {
  ...
  public static String nameFromNumber(int i) {
    TcpPort namedPort = NUMBER_TO_PORT_MAP.get(i);
    if (namedPort == null) {
