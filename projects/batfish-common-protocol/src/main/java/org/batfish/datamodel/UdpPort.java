...
public enum UdpPort {
  ...
  private static final Map<Integer, UdpPort> NUMBER_TO_PORT_MAP = buildNumberToPortMap();

  private static Map<Integer, UdpPort> buildNumberToPortMap() {
    ImmutableMap.Builder<Integer, UdpPort> map = ImmutableMap.builder();
    for (UdpPort protocol : values()) {
      map.put(protocol._number, protocol);
    }
    return map.build();
  }

  public static UdpPort fromNumber(int number) {
    UdpPort ret = NUMBER_TO_PORT_MAP.get(number);
    if (ret == null) {
  ...
  public static String nameFromNumber(int i) {
    UdpPort namedPort = NUMBER_TO_PORT_MAP.get(i);
    if (namedPort == null) {
