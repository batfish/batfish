...
public class QuestionHelper {
  ...
  public enum MacroType {
    ...
    private static final Map<String, MacroType> _map = buildMap();
  ...
  public static IQuestion getReachabilityQuestion(
      String dstIp,
      String protocolStr,
      String ingressNodeRegex,
      ForwardingAction action,
      Map<String, Supplier<Question>> questions) {

    if (!questions.containsKey(IReachabilityQuestion.NAME)) {
      throw new BatfishException(
          "Reachability question not found. Did you include the questions plugins directory in "
              + "your JVM arguments?");
    }

    IReachabilityQuestion question =
        (IReachabilityQuestion) questions.get(IReachabilityQuestion.NAME).get();

    question.setDstIps(new TreeSet<>(Collections.singleton(new IpWildcard(new Ip(dstIp)))));
    ...
    SortedSet<Protocol> protocols =
        new TreeSet<>(Collections.singleton(Protocol.fromString(protocolStr)));
    if (inverted) {
  ...
  public static String resolveMacro(
      String macroName, String paramsLine, Map<String, Supplier<Question>> questions) {
      ...
      case CHECKPROTECTION:
        {
          ...
          return getReachabilityQuestion(
                  dstIp, protocol, ingressNodeRegex, ForwardingAction.ACCEPT, questions)
              .toJsonString();
        }
      case CHECKREACHABILITY:
        {
          ...
          return getReachabilityQuestion(
                  dstIp, protocol, ingressNodeRegex, ForwardingAction.DROP, questions)
              .toJsonString();
        }
      case TRACEROUTE:
        {
          ...
          if (words.length < 2 || words.length > 3) {
            throw new BatfishException(
                "Incorrect usage for traceroute macro. "
                    + "Should be:\n #traceroute <srcNode> <dstip> [<protocol>]");
          }
          ITracerouteQuestion question =
              (ITracerouteQuestion) questions.get(ITracerouteQuestion.NAME).get();
