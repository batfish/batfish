...
public class Interface extends NamedStructure {
   ...
   public String getIFString(int indentLevel) {
	   String retString = Util.getIndentString(indentLevel) + "Interface " + getName();
	   
	   retString += String.format("\n%sActive %s", Util.getIndentString(indentLevel+1),  _active);
	   retString += String.format("\n%sIp %s", Util.getIndentString(indentLevel+1), _ip);
	   retString += String.format("\n%sSubnet %s", Util.getIndentString(indentLevel+1), _subnet);
	   retString += String.format("\n%sBandwidth %s", Util.getIndentString(indentLevel+1), _bandwidth);

	   retString += String.format("\n%sSwitchportMode %s", Util.getIndentString(indentLevel+1), _switchportMode);
	   retString += String.format("\n%sSwitchportEncapsulationType %s", Util.getIndentString(indentLevel+1), _switchportTrunkEncapsulation);

	   retString += String.format("\n%sOspfArea %s", Util.getIndentString(indentLevel+1), _ospfArea);
	   retString += String.format("\n%sOspfCost %s", Util.getIndentString(indentLevel+1), _ospfCost);
	   retString += String.format("\n%sOspfDeadInterval %s", Util.getIndentString(indentLevel+1), _ospfDeadInterval);
	   retString += String.format("\n%sOspfHelloMultiplier %s", Util.getIndentString(indentLevel+1), _ospfHelloMultiplier);
	   	   
	   retString += String.format("\n%sAccessVlan %s", Util.getIndentString(indentLevel+1), _accessVlan);
	   retString += String.format("\n%sNativeVlan %s", Util.getIndentString(indentLevel+1), _nativeVlan);

	   if (_allowedVlans.size() > 0) {
		   retString += "\n" + Util.getIndentString(indentLevel+1) + "AllowedVlans";
	   ...
	   if (_incomingFilter != null) {
		   retString += "\n" + Util.getIndentString(indentLevel+1) + "IncomingFilter " + _incomingFilter.getName();		   	   
	   }

	   if (_outgoingFilter != null) {
		   retString += "\n" + Util.getIndentString(indentLevel+1) + "OutgoingFilter " + _outgoingFilter.getName();		   	   
	   }
	   
	   if (_secondaryIps.size() > 0) {
		   for (Map.Entry<Ip, Ip> entry : _secondaryIps.entrySet()) {
			   retString += "\n" + Util.getIndentString(indentLevel + 1) + String.format("SecondaryIp %s %s",  entry.getKey(), entry.getValue());
		   }		   
	   }
   ...
   public boolean sameParseTree(Interface face, Configuration lhsC,
      ...
      if (_allowedVlans.size() != face._allowedVlans.size()) {
         System.out.println("Interface:" + _name + ":allowedVlans:Size " + prefix);
         finalRes = false;
      }
      else {
         for (int i = 0; i < _allowedVlans.size(); i++) {
            res = (_allowedVlans.get(i).toString().equals(face._allowedVlans
                        .get(i).toString()));
            if (res == false) {
               System.out.println("Interface:" + _name + ":allowedVlans "+prefix);
               finalRes = res;
            }
         }
      }
      ...
      if ((_incomingFilter != null) && (face._incomingFilter != null)) {
         ...
         res =  (_incomingFilter.sameParseTree(face._incomingFilter, "Interface:" + _name + ":IncomingFilter "+prefix, true));
      }
      else {
         res = (_incomingFilter == null)
               && (face._incomingFilter == null);
         if (res == false) {
            System.out.println("Interface:" + _name + ":IncomingFilter "+prefix);
         }
      }
      if (res == false) {
         finalRes = res;
      ...
      if ((_outgoingFilter != null) && (face._outgoingFilter != null)) {
         ...
         res =  (_outgoingFilter.sameParseTree(face._outgoingFilter, "Interface:" + _name + ":OutgoingFilter "+prefix, true));
      }
      else {
         res = (_outgoingFilter == null)
               && (face._outgoingFilter == null);
         if (res == false) {
            System.out.println("Interface:" + _name + ":OutgoingFilter "+prefix);
      ...
      for (Ip ip : _secondaryIps.keySet()) {
         ...
         if (rhs == null) {
            System.out.println("Interface:" + _name + ":secondIp:NullRhs "+prefix);
            finalRes = false;
         }
         else {
            res = lhs.equals(rhs);
            if (res == false) {
               System.out.println("Interface:" + _name + ":secondIp "+prefix);
               finalRes = res;
            }
         }
      }
