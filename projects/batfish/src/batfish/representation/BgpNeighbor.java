...
public class BgpNeighbor implements Serializable {
   ...
   public boolean sameParseTree(BgpNeighbor neighbor, String prefix) {
      ...
      /*
       * if(_groupName != null){ res = res &&
       * (_groupName.equals(neighbor._groupName)); }else{ res = res &&
       * (neighbor._groupName == null); } if(res == false){
       * System.out.print("BGPNeighbor:GroupName "); return res; }
       */
      ...
      if (_generatedRoutes.size() != neighbor._generatedRoutes.size()) {
         ...
         for (GeneratedRoute lhs : _generatedRoutes) {
            ...
            for (GeneratedRoute rhs : neighbor._generatedRoutes) {
               if (lhs.equals(rhs)) {
                  res = lhs
                        .sameParseTree(rhs, "BGPNeighbor:GenRoute " + prefix);
                  found = true;
                  if (res == false) {
                     finalRes = res;
                  }
         ...
         for (PolicyMap lhs : _inboundPolicyMaps) {
            ...
            for (PolicyMap rhs : neighbor._inboundPolicyMaps) {
               if (lhs.getMapName().equals(rhs.getMapName())) {
                  res = lhs.sameParseTree(rhs, "BGPNeighbor:inboundPoliMap "
                        + prefix);
                  found = true;
                  if (res == false) {
                     finalRes = res;
            ...
            if (found == false) {
               System.out.println("BGPNeighbor:inboundPoliMap:NotFound "
                     + prefix);
               finalRes = false;
            }
         }
      ...
      if (_originationPolicies.size() != neighbor._originationPolicies.size()) {
         ...
         for (PolicyMap lhs : _originationPolicies) {
            ...
            for (PolicyMap rhs : neighbor._originationPolicies) {
               if (lhs.getMapName().equals(rhs.getMapName())) {
                  res = lhs
                        .sameParseTree(rhs, "BGPNeighbor:origPoli " + prefix);
                  found = true;
                  if (res == false) {
                     finalRes = res;
                  }
      ...
      if (_outboundPolicyMaps.size() != neighbor._outboundPolicyMaps.size()) {
         ...
         for (PolicyMap lhs : _outboundPolicyMaps) {
            ...
            for (PolicyMap rhs : neighbor._outboundPolicyMaps) {
               if (lhs.getMapName().equals(rhs.getMapName())) {
                  res = lhs.sameParseTree(rhs, "BGPNeighbor:outboundPoliMap "
                        + prefix);
                  found = true;
                  if (res == false) {
                     finalRes = res;
            ...
            if (found == false) {
               System.out.println("BGPNeighbor:outboundPoliMap:NotFound "
                     + prefix);
               finalRes = false;
            }
         }
