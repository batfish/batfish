...
public class BgpProcess {
   ...
   public boolean sameParseTree(BgpProcess bgp, String prefix) {
      ...
      if(_bgpNeighbors.size() != bgp._bgpNeighbors.size()){
         System.out.println("BGP:BGPNeighbor:Size"+_bgpNeighbors.size()+":"+bgp._bgpNeighbors.size()+" "+prefix);
         finalRes = res;
      }else{
      for(BgpNeighbor lhs : _bgpNeighbors.values()){
         BgpNeighbor rhs = bgp._bgpNeighbors.get(lhs.getAddress().toString());
         if(rhs == null){
            System.out.println("BGP:BGPNeighbor:RhsNull "+prefix);
            finalRes = false;
         }else{
            res = lhs.sameParseTree(rhs, "BGP:BGPNeighbor:ParseTree "+prefix);
            if(res == false){
               finalRes = res;
            }            
         }
