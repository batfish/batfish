...
public abstract class BgpRibAbstract<R extends BgpRoute> extends AbstractRib<R> {
  ...
  public RibDelta<R> mergeRouteGetDelta(R route) {
    RibDelta<R> delta = mergeRouteGetDelta(route);
  ...
  public RibDelta<R> removeRouteGetDelta(R route, Reason reason) {
    RibDelta<R> delta = removeRouteGetDelta(route, reason);
    if (!delta.isEmpty()) {
      delta.getPrefixes().forEach(this::selectBestPath);
      delta
          .getActions()
