...
public final class BgpProtocolHelper {
   ...
   * common path: most routes carry no extended communities at all, so it bails on {@link
   * CommunitySet#hasExtendedCommunities()} before materializing any subset, and only builds a new
   * set (the one case that must allocate) when a non-transitive extended community is found.
   */
  @VisibleForTesting
  static @Nonnull CommunitySet removeNonTransitiveExtendedCommunities(CommunitySet communities) {
    if (!communities.hasExtendedCommunities()) {
      return communities;
    }
    for (ExtendedCommunity ec : communities.getExtendedCommunities()) {
      if (!ec.isTransitive()) {
