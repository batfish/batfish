...
public final class BgpProtocolHelper {
  ...
  public static <R extends BgpRoute<B, R>, B extends BgpRoute.Builder<B, R>>
    ...
    if (!(route instanceof EvpnRoute<?, ?>)) {
      ...
      // todo perhaps need to set next hop for type 3?
      importBuilder
   ...
   *
   * <p>Sets next hop for non-EVPN routes only. EVPN routes' NH should be set by {@link
   * #setEvpnNhPostExport}.
   *
  ...
  /**
   * Sets next hop on EVPN route builder in preparation for export. Preserves original next hop
   * unless it is {@link NextHopDiscard}, which indicates the EVPN route was originated; in this
   * case the route builder is given a {@link NextHopVtep}.
   *
   ...
   */
  public static <B extends EvpnRoute.Builder<?, ?>> boolean setEvpnNhPostExport(
      B routeBuilder, EvpnAddressFamily af, NextHop originalRouteNh, int originalRouteVni) {
  ...
  static <R extends BgpRoute<B, R>, B extends BgpRoute.Builder<B, R>>
    ...
    //  If so, this should step be skipped
    if (!isEvpn && routeBuilder.getNextHopIp().equals(UNSET_ROUTE_NEXT_HOP_IP)) {
      if (isEbgp) {
