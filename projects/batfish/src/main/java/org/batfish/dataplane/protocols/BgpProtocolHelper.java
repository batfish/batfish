...
public final class BgpProtocolHelper {
   ...
   * and large communities, and transitive extended communities, are preserved.
   *
   * <p>This runs on every BGP route export. It avoids streams (hot path), but does not bother with
   * a separate scan to preserve the input instance: {@link CommunitySet#of} interns its result, so
   * when nothing is removed the cache returns the same interned {@link CommunitySet}.
   */
  @VisibleForTesting
  static @Nonnull CommunitySet removeNonTransitiveExtendedCommunities(CommunitySet communities) {
    ...
    ImmutableSet.Builder<Community> kept = ImmutableSet.builderWithExpectedSize(all.size());
    for (Community c : all) {
      if (!(c instanceof ExtendedCommunity ec) || ec.isTransitive()) {
        kept.add(c);
      }
    }
