...
public class AciPolUniDeserializer extends JsonDeserializer<AciPolUniInternal>
    ...
    if (childrenNode != null && childrenNode.isArray()) {
      ...
      for (JsonNode childNode : childrenNode) {
        ...
        if (childNode.has("fvTenant")) {
          ...
          child.setFabricInst(parseFabricInst(childNode.get("fabricInst")));
        } else if (childNode.has("ctrlrInst")) {
          child.setCtrlrInst(parseCtrlrInst(childNode.get("ctrlrInst")));
        }
        children.add(child);
      }
  ...
  private static AciCtrlrInst parseCtrlrInst(JsonNode node) throws IOException {
    ...
    if (childrenNode != null && childrenNode.isArray()) {
      ...
      for (JsonNode childNode : childrenNode) {
        ...
        if (childNode.has("fabricNodeIdentPol")) {
          child.setFabricNodeIdentPol(parseFabricNodeIdentPol(childNode.get("fabricNodeIdentPol")));
        }
        children.add(child);
      }
  ...
  private static AciFabricInst parseFabricInst(JsonNode node) throws IOException {
  ...
  private static AciFabricNodeIdentPol parseFabricNodeIdentPol(JsonNode node) throws IOException {
