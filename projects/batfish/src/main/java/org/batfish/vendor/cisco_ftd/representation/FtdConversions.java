...
public final class FtdConversions {
  ...
  public static @Nonnull IpsecPhase2Proposal toIpsecPhase2Proposal(
      FtdIpsecTransformSet transformSet) {
  ...
  public static Map<String, IpsecPeerConfig> toIpsecPeerConfigs(
    ...
    for (org.batfish.datamodel.Interface iface : referencingInterfaces) {
      ...
      if (addr == null) {
        w.redFlagf(
            "Interface %s has crypto map %s but no IP address", iface.getName(), cryptoMapName);
        continue;
      }

      String peerName =
          String.format(
              "~IPSEC_PEER_CONFIG:%s:%d:%s~",
              cryptoMapName, entry.getSequenceNumber(), iface.getName());

      IpsecPeerConfig.Builder<?, ?> builder;
      if (entry.getPeer() != null) {
        // Static peer
        builder = IpsecStaticPeerConfig.builder().setDestinationAddress(entry.getPeer());
      } else {
  ...
  static @Nullable IpAccessList createAclWithSymmetricalLines(IpAccessList ipAccessList) {
    ...
    for (AclLine line : ipAccessList.getLines()) {
      ...
      HeaderSpace reversedHeaderSpace =
          originalHeaderSpace.toBuilder()
