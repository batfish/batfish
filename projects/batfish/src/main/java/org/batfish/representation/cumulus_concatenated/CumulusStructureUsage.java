...
public enum CumulusStructureUsage implements StructureUsage {
  ...
  private static final Map<String, CumulusStructureUsage> MAP = initMap();

  public static CumulusStructureUsage fromFrrStructureUsage(
      @Nonnull FrrStructureUsage frrStructureUsage) {
    checkArgument(MAP.containsKey(frrStructureUsage.getDescription()));
    return MAP.get(frrStructureUsage.getDescription());
  }

  private static Map<String, CumulusStructureUsage> initMap() {
    ImmutableMap.Builder<String, CumulusStructureUsage> map = ImmutableMap.builder();
    for (CumulusStructureUsage value : CumulusStructureUsage.values()) {
