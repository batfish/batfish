 ...
 *
 * <ul>
 *   <li>Scalar/numeric families (origin, metric, local-preference, next-hop, etc.) use last-wins
 *       semantics: only the last action is retained.
 ...
 *       emitted. Exact duplicates are deduplicated.
 * </ul>
 *
...
public final class PsThens implements Serializable {
  ...
  public @Nonnull List<String> addPsThen(PsThen then) {
    ...
    List<String> result;
    if (isCumulativeFamily(family)) {
      result = addCumulative(then, family);
    } else {
      result = setLastWins(then, family);
    }
  ...
  private static boolean isCumulativeFamily(String family) {
    switch (family) {
      case "tunnel-attribute set":
        return true;
      default:
        return false;
    }
  ...
  private @Nonnull List<String> addCommunityAction(PsThen then) {
    // Exact duplicate of any prior community action: dedup, drop the new one
    if (_communityActions.contains(then)) {
  ...
  private @Nonnull List<String> addCumulative(PsThen then, String family) {
    List<PsThen> existing = _familyActions.computeIfAbsent(family, k -> new ArrayList<>(1));
