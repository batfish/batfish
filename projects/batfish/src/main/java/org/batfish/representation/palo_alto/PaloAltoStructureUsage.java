...
public enum PaloAltoStructureUsage implements StructureUsage {
  ...
  BGP_PEER_LOCAL_ADDRESS_IP("bgp peer local-address ip"),
  BGP_PEER_ADDRESS("bgp peer peer-address"),
  ETHERNET_AGGREGATE_GROUP("ethernet aggregate-group"),
  IKE_GATEWAY_IKE_CRYPTO_PROFILE("ike gateway protocol ike-crypto-profile"),
  IKE_GATEWAY_LOCAL_ADDRESS_FLOATING_IP("ike gateway local-address floating-ip"),
  IKE_GATEWAY_LOCAL_ADDRESS_INTERFACE("ike gateway local-address interface"),
  IKE_GATEWAY_LOCAL_ADDRESS_IP("ike gateway local-address ip"),
  IKE_GATEWAY_PEER_ADDRESS("ike gateway peer-address"),
  IMPORT_INTERFACE("import network interface"),
  IPSEC_TUNNEL_IKE_GATEWAY("tunnel ipsec auto-key ike-gateway"),
  IPSEC_TUNNEL_IPSEC_CRYPTO_PROFILE("tunnel ipsec auto-key ipsec-crypto-profile"),
  IPSEC_TUNNEL_TUNNEL_INTERFACE("tunnel ipsec tunnel-interface"),
