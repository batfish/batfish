...
public class AwsConfiguration extends VendorConfiguration {
  ...
  /**
   * Sets attributes in configs where data must be obtained from other JSON files. Example:
   * VpnConnections references information in both Transit Gateways and Customer Gateways.
   */
        ...
        for (VpnConnection vpns : region.getVpnConnections().values()) {
          ...
          for (IpsecTunnel tunnel : vpns.getIpsecTunnels()) {
            ...
            if (gatewayType.equals(VpnConnection.GatewayType.TRANSIT)) {
              // get tgw
              tunnel.setVgwBgpAsn(
                  region.getTransitGateways().get(gatewayId).getOptions().getAmazonSideAsn());
            ...
            tunnel.setcgwOutsideAddress(cgw.getIpAddress());
          }
        }
      }
    }
