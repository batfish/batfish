...
public final class FwFromInterfaceGroup implements FwFrom {
  ...
  private void addIfMatching(Interface iface, ImmutableSet.Builder<String> matchingInterfaces) {
    ...
    // Interface-group-except matches interfaces assigned to other groups. Explicitly selecting
    // those interfaces avoids matching ungrouped interfaces or device-originated traffic.
    if (group != null && (_except ? !_group.includes(group) : _group.includes(group))) {
