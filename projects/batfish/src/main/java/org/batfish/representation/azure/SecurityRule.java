...
public class SecurityRule extends Resource implements Serializable {
  ...
  public static class Properties implements Serializable {
    ...
    public Properties(
      ...
      _sourcePortRanges =
          sourcePortRanges.stream().map(Properties::getSubRange).collect(Collectors.toSet());

      _destinationPortRanges =
          destinationPortRanges.stream().map(Properties::getSubRange).collect(Collectors.toSet());
    }

    /** convert {@link String} to {@link IpProtocol} */
    private static IpProtocol getProtocol(String protocol) {
      if (protocol.equals("*")) return IpProtocol.ANY_0_HOP_PROTOCOL;
    ...
    private static IpSpace parseIpSpace(String prefix) {
      ...
      if (prefix.equals("Internet")) return InternetIpSpace;
      if (prefix.equals("VirtualNetwork")) return VirtualNetworkIpSpace;
