...
public class FtdConfiguration extends VendorConfiguration {
  ...
  public void addCryptoMapInterfaceBinding(@Nonnull String cryptoMap, @Nonnull String ifaceName) {
    _cryptoMapInterfaceBindings
        .computeIfAbsent(cryptoMap, key -> new HashSet<>())
        .add(ifaceName);
  }
  ...
  private void convertIkev2Policies(Configuration c) {
    ...
    for (FtdIkev2Policy repPolicy : _ikev2Policies.values()) {
      ...
      viPolicy.setIkePhase1Proposals(viProposals.stream().map(IkePhase1Proposal::getName).collect(Collectors.toList()));
  ...
  private void convertCryptoMaps(Configuration c) {
    ...
    _interfaces.forEach((ifName, repIface) -> {
    ...
    for (Map.Entry<String, Set<String>> binding : _cryptoMapInterfaceBindings.entrySet()) {
      ...
      for (String interfaceNameOrNameif : binding.getValue()) {
        ...
        String interfaceName = nameifToInterface.getOrDefault(interfaceNameOrNameif, interfaceNameOrNameif);
    ...
    for (FtdCryptoMapSet repMapSet : _cryptoMaps.values()) {
      for (FtdCryptoMapEntry entry : repMapSet.getEntries().values()) {
        String policyName = String.format("~IPSEC_PHASE2_POLICY:%s:%d~", repMapSet.getName(),
            entry.getSequenceNumber());
        phase2Policies.put(policyName, FtdConversions.toIpsecPhase2Policy(entry));
        peerConfigs.putAll(FtdConversions.toIpsecPeerConfigs(c, entry, repMapSet.getName(), policyName, _w));
      }
    }
  ...
  private void createZones(Configuration c) {
    ...
    zoneInterfaces.forEach((name, interfaces) -> {
  ...
  private void convertBgpProcess(FtdBgpProcess bgp, Configuration c) {
    ...
    BgpProcess proc = BgpProcess.builder()
  ...
  private void convertAccessList(String name, FtdAccessList acl, Configuration c) {
    ...
    List<AclLine> lines = acl.getLines().stream()
        .filter(line -> !line.isInactive()) // Skip inactive lines
        .map(line -> convertAccessListLine(line, nameifToInterface))
        .filter(line -> line != null) // Skip unsupported lines
        .collect(Collectors.toList());

    IpAccessList ipAccessList = IpAccessList.builder()
        .setName(name)
        .setLines(lines)
        .build();
  ...
  private AclLine convertAccessListLine(FtdAccessListLine line, Map<String, String> nameifToInterface) {
    ...
    if (line.getInterfaceName() != null) {
      String interfaceName = nameifToInterface.getOrDefault(line.getInterfaceName(), line.getInterfaceName());
      combinedMatch = AclLineMatchExprs.and(
          combinedMatch,
          AclLineMatchExprs.matchSrcInterface(interfaceName));
    }
    ...
    for (FtdServiceObjectGroupMember member : group.getMembers()) {
      switch (member.getType()) {
        ...
        case PORT_OBJECT:
          ...
          break;
        case GROUP_OBJECT:
          ServiceObjectGroupMatch nested = resolveServiceObjectGroupMatch(member.getObjectName(), visited);
          if (nested != null) {
            protocols.addAll(nested.getProtocols());
            if (nested.isAnyPort()) {
              anyPort = true;
            } else if (nested.getPorts() != null) {
              ports = ports == null ? nested.getPorts() : IntegerSpace.unionOf(ports, nested.getPorts());
            }
          }
          break;
        default:
  ...
  private void applyAccessGroups(Configuration c) {
    ...
    Map<String, String> nameifToInterface = new HashMap<>();
    _interfaces.forEach((name, repIface) -> {
      if (repIface.getNameif() != null) {
  ...
  private void applyNatRules(Configuration c) {
    ...
    for (FtdNatRule rule : _natRules) {
      String srcInterface = nameifToInterface.getOrDefault(rule.getSourceInterface(), rule.getSourceInterface());
      String dstInterface = nameifToInterface.getOrDefault(rule.getDestinationInterface(),
          rule.getDestinationInterface());
      ...
      if (rule.getSourceTranslation() != null) {
        Transformation.Builder t = toSourceNatTransformation(
      ...
      if (rule.getDestinationTranslation() != null) {
        Transformation.Builder t = toDestinationNatTransformation(
    ...
    incomingByInterface.forEach((ifaceName, builders) -> {
      ...
      iface.setIncomingTransformation(chainWithOrElse(chain, iface.getIncomingTransformation()));
    });

    outgoingByInterface.forEach((ifaceName, builders) -> {
      org.batfish.datamodel.Interface iface = c.getAllInterfaces().get(ifaceName);
      if (iface == null) {
        return;
      }
      Transformation chain = toTransformationChain(builders);
      iface.setOutgoingTransformation(chainWithOrElse(chain, iface.getOutgoingTransformation()));
    });
  }
  ...
  private void applySecurityLevelDefaults(Configuration c) {
    for (Map.Entry<String, Interface> entry : _interfaces.entrySet()) {
      ...
      ExprAclLine line = ExprAclLine.builder()
      ...
      IpAccessList acl = IpAccessList.builder()
          .setName(aclName)
          .setLines(ImmutableList.of(line))
          .build();
  ...
  private @Nullable Transformation.Builder toSourceNatTransformation(
    ...
    if (service != null) {
      ...
      if (translation != null && translation._real._port != null && translation._mapped._port != null) {
  ...
  private @Nullable TransformationStep buildDestinationNatStep(
    ...
    if (mapped._prefix != null && real._prefix != null
        && mapped._prefix.getPrefixLength() == real._prefix.getPrefixLength()
  ...
  private @Nullable Transformation.Builder toManualTwiceNatOutgoing(
    ...
    if (srcReal == null || dstReal == null || srcReal._matchSpace == null || dstReal._matchSpace == null) {
      return null;
    }
    ImmutableList.Builder<TransformationStep> steps = ImmutableList.builder();
    if (srcMapped != null) {
  ...
  private @Nullable ServiceTranslation parseServiceTranslation(FtdNatService service) {
    ...
    if (real == null) {
      real = new ServiceSpec(parseProtocol(realSpec), realPorts.isEmpty() ? null : realPorts.get(0));
    }
    if (mapped == null) {
      mapped = new ServiceSpec(parseProtocol(mappedSpec), mappedPorts.isEmpty() ? null : mappedPorts.get(0));
    }
  ...
  private @Nullable NatAddressInfo resolveNatAddressInfo(FtdNatAddress address) {
    return address.accept(new FtdNatAddress.Visitor<>() {
      @Override
      public NatAddressInfo visitFtdNatAddressIp(FtdNatAddress.FtdNatAddressIp ftdNatAddressIp) {
        Ip ip = ftdNatAddressIp.getIp();
        return new NatAddressInfo(ip.toIpSpace(), ip.toPrefix(), null, null);
      }

      @Override
      public NatAddressInfo visitFtdNatAddressName(FtdNatAddress.FtdNatAddressName ftdNatAddressName) {
  ...
  private @Nullable Transformation toTransformationChain(List<OptionalTransformationBuilder> builders) {
  ...
  private void convertOspfProcesses(Configuration c) {
    _ospfProcesses.forEach((name, ospf) -> {
      c.getVrfs()
  ...
  private OspfProcess toOspfProcess(String name, FtdOspfProcess ospf) {
    ...
    // Build areas
    Map<Long, OspfArea> builtAreas = areas.entrySet().stream()
  ...
  private void convertInterface(org.batfish.representation.cisco_ftd.Interface repIface,
      Configuration c) {
    String ifName = repIface.getName();
    org.batfish.datamodel.Interface.Builder ib = org.batfish.datamodel.Interface.builder()
        .setOwner(c)
        .setName(ifName)
    ...
    _cryptoMapInterfaceBindings.forEach((cryptoMap, ifaceNames) -> {
  ...
  private void applyMpfMetadata(Configuration c) {
    ...
    if (!_classMaps.isEmpty()) {
      Service classMaps = mpfService.getSubservices().computeIfAbsent("class-map", k -> new Service());
      _classMaps.keySet().forEach(name -> classMaps.getSubservices().put(name, new Service()));
    }
    if (!_policyMaps.isEmpty()) {
      Service policyMaps = mpfService.getSubservices().computeIfAbsent("policy-map", k -> new Service());
      _policyMaps.keySet().forEach(name -> policyMaps.getSubservices().put(name, new Service()));
    }
    if (!_servicePolicies.isEmpty()) {
      Service servicePolicies = mpfService.getSubservices().computeIfAbsent("service-policy", k -> new Service());
  ...
  private void convertMpfToAcls(Configuration c) {
    ...
    _classMaps.forEach((name, classMap) -> {
    ...
    _policyMaps.forEach((name, policyMap) -> {
      List<AclLine> lines = new ArrayList<>();
