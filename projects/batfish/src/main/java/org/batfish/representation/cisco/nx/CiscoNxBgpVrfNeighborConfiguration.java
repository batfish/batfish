...
public class CiscoNxBgpVrfNeighborConfiguration implements Serializable {
  ...
  private void inheritFrom(
      ...
      Warnings warnings,
      CiscoNxBgpVrfNeighborConfiguration peer) {
  ...
  public void doInherit(CiscoNxBgpGlobalConfiguration nxBgpGlobal, Warnings warnings) {
    if (_doneInheriting) {
      return;
    }
    ...
    CiscoNxBgpVrfNeighborConfiguration peerSession =
    ...
    if (peer != null) {
      ...
      for (Map.Entry<String, CiscoNxBgpVrfNeighborAddressFamilyConfiguration> af :
          peer._addressFamilies.entrySet()) {
        getOrCreateAddressFamily(af.getKey()).inheritFrom(nxBgpGlobal, warnings, af.getValue());
      }
    }

    for (CiscoNxBgpVrfNeighborAddressFamilyConfiguration af : _addressFamilies.values()) {
