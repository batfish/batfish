...
import org.batfish.grammar.flatjuniper.FlatJuniperParser.Hierarchy_elementContext;
...
public class InsertDeleteApplicator extends FlatJuniperParserBaseListener
  ...
  public void exitProtect_line(Protect_lineContext ctx) {
    List<String> path = new ArrayList<>(ctx.hierarchy_element().size());
    for (Hierarchy_elementContext element : ctx.hierarchy_element()) {
      path.add(element.getText());
    }
    _protectedPaths.add(ImmutableList.copyOf(path));
    _dirty = true;
  }
  ...
  private boolean isBlockedReplacement(List<String> path) {
    for (List<String> blockedPath : _blockedReplacePaths) {
      if (isPrefix(blockedPath, path)) {
        return true;
      }
    }
    return false;
  ...
  private boolean isProtected(List<String> path) {
    for (List<String> protectedPath : _protectedPaths) {
      if (isPrefix(protectedPath, path) || isPrefix(path, protectedPath)) {
        return true;
      }
    }
    return false;
