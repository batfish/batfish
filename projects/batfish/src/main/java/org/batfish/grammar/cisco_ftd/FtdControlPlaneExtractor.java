...
import org.batfish.grammar.cisco_ftd.FtdParser.Hostname_stanzaContext;
import org.batfish.grammar.cisco_ftd.FtdParser.Interface_stanzaContext;
import org.batfish.grammar.cisco_ftd.FtdParser.If_descriptionContext;
import org.batfish.grammar.cisco_ftd.FtdParser.If_shutdownContext;
import org.batfish.grammar.cisco_ftd.FtdParser.If_no_shutdownContext;
import org.batfish.grammar.cisco_ftd.FtdParser.If_nameifContext;
import org.batfish.grammar.cisco_ftd.FtdParser.If_ip_addressContext;
import org.batfish.grammar.cisco_ftd.FtdParser.If_security_levelContext;
import org.batfish.grammar.cisco_ftd.FtdParser.If_vlanContext;
import org.batfish.grammar.cisco_ftd.FtdParser.If_vrfContext;
import org.batfish.grammar.cisco_ftd.FtdParser.Access_list_stanzaContext;
...
import org.batfish.grammar.cisco_ftd.FtdParser.Acl_address_specContext;
import org.batfish.grammar.cisco_ftd.FtdParser.Access_list_actionContext;
import org.batfish.grammar.cisco_ftd.FtdParser.Port_specContext;
import org.batfish.grammar.cisco_ftd.FtdParser.Object_stanzaContext;
import org.batfish.grammar.cisco_ftd.FtdParser.Object_hostContext;
import org.batfish.grammar.cisco_ftd.FtdParser.Object_subnetContext;
import org.batfish.grammar.cisco_ftd.FtdParser.Object_fqdnContext;
import org.batfish.grammar.cisco_ftd.FtdParser.Object_group_stanzaContext;
import org.batfish.grammar.cisco_ftd.FtdParser.Og_network_objectContext;
import org.batfish.grammar.cisco_ftd.FtdParser.Og_group_objectContext;
...
import org.batfish.grammar.cisco_ftd.FtdParser.Class_map_tailContext;
...
import org.batfish.representation.cisco_ftd.FtdAccessListAddressSpecifier;
...
import org.batfish.representation.cisco_ftd.FtdServiceObjectGroupMember;
import org.batfish.representation.cisco_ftd.FtdClassMap;
import org.batfish.representation.cisco_ftd.FtdPolicyMap;
import org.batfish.representation.cisco_ftd.FtdServicePolicy;
import org.batfish.representation.cisco_ftd.Interface;
import org.batfish.representation.cisco_ftd.FtdNatRule;
import org.batfish.representation.cisco_ftd.FtdNatSource;
import org.batfish.representation.cisco_ftd.FtdNatAddress;
import org.batfish.representation.cisco_ftd.FtdAccessGroup;
import org.batfish.representation.cisco_ftd.FtdNatDestination;
import org.batfish.representation.cisco_ftd.FtdNatService;
import org.batfish.representation.cisco_ftd.FtdRoute;
import org.batfish.representation.cisco_ftd.FtdOspfProcess;
import org.batfish.representation.cisco_ftd.FtdOspfNetwork;
import org.batfish.vendor.VendorConfiguration;
import org.batfish.grammar.cisco_ftd.FtdParser.Nat_stanzaContext;
import org.batfish.grammar.cisco_ftd.FtdParser.Nat_sourceContext;
...
import org.batfish.grammar.cisco_ftd.FtdParser.Router_bgp_stanzaContext;
...
import org.batfish.grammar.cisco_ftd.FtdParser.Crypto_mapContext;
import org.batfish.grammar.cisco_ftd.FtdParser.Crypto_map_interfaceContext;
import org.batfish.grammar.cisco_ftd.FtdParser.Cm_match_addressContext;
import org.batfish.grammar.cisco_ftd.FtdParser.Cm_setContext;
import org.batfish.grammar.cisco_ftd.FtdParser.Crypto_ikev2_policyContext;
import org.batfish.grammar.cisco_ftd.FtdParser.Ikev2_policy_attrContext;
import org.batfish.grammar.cisco_ftd.FtdParser.Tunnel_group_stanzaContext;
import org.batfish.grammar.cisco_ftd.FtdParser.Tg_ikev2_attrContext;
...
import org.batfish.representation.cisco_ftd.FtdStructureType;
import org.batfish.representation.cisco_ftd.FtdStructureUsage;
...
/**
 * Parse tree extractor that walks the FTD parse tree and populates an
 * {@link FtdConfiguration}
 * object.
 */
...
public class FtdControlPlaneExtractor extends FtdParserBaseListener
  ...
  public void exitIf_description(If_descriptionContext ctx) {
    if (_currentInterface != null && ctx.description_line() != null
  ...
  public void enterAccess_list_stanza(Access_list_stanzaContext ctx) {
    ...
    _currentAcl = _configuration.getAccessLists().computeIfAbsent(
        aclName,
        k -> new FtdAccessList(k));
  ...
  public void exitAccess_list_stanza(Access_list_stanzaContext ctx) {
    if (_currentAcl != null && _currentAclName != null) {
      if (ctx.acl_remark() != null) {
        ...
        String remark = remarkCtx.remark_text != null
            ? remarkCtx.remark_text.getText().trim()
            : "";
  ...
  public void exitAcl_extended(Acl_extendedContext ctx) {
    if (_currentAcl != null && _currentAclName != null) {
      ...
      FtdAccessListLine line = FtdAccessListLine.createExtended(
          _currentAclName, action, protocol, srcSpec, dstSpec);
  ...
  @Override
  public void exitAcl_implicit_extended(org.batfish.grammar.cisco_ftd.FtdParser.Acl_implicit_extendedContext ctx) {
  ...
  public void exitAcl_advanced(Acl_advancedContext ctx) {
    if (_currentAcl != null && _currentAclName != null) {
      ...
      FtdAccessListLine line = FtdAccessListLine.createAdvanced(
          _currentAclName, action, protocol, srcSpec, dstSpec);
  ...
  public void enterCrypto_ikev2_policy(Crypto_ikev2_policyContext ctx) {
    ...
    _currentIkev2Policy = _configuration.getIkev2Policies().computeIfAbsent(
        priority, FtdIkev2Policy::new);
  ...
  public void exitIkev2_policy_attr(Ikev2_policy_attrContext ctx) {
    ...
    if (ctx.ENCRYPTION() != null && ctx.enc_algs != null) {
      String text = ctx.enc_algs.stream().map(Token::getText).collect(java.util.stream.Collectors.joining("")).trim();
      ...
      String text = ctx.int_algs.stream().map(Token::getText).collect(java.util.stream.Collectors.joining("")).trim();
      ...
      String text = ctx.prf_algs.stream().map(Token::getText).collect(java.util.stream.Collectors.joining("")).trim();
      IkeHashingAlgorithm alg = parseIkeHashingAlgorithm(text);
      if (alg != null) {
        _currentIkev2Policy.getPrfAlgorithms().add(alg);
      }
    } else if (ctx.GROUP() != null && ctx.dh_groups != null) {
      String text = ctx.dh_groups.stream().map(Token::getText).collect(java.util.stream.Collectors.joining("")).trim();
  ...
  public void enterTunnel_group_stanza(Tunnel_group_stanzaContext ctx) {
    String name = ctx.name_parts.stream()
        .map(token -> token.getText())
        .collect(java.util.stream.Collectors.joining(" "))
        .trim();
    _currentTunnelGroup = _configuration.getTunnelGroups().computeIfAbsent(
        name, FtdTunnelGroup::new);
  ...
  public void exitTg_ikev2_attr(Tg_ikev2_attrContext ctx) {
    ...
    if (ctx.PRE_SHARED_KEY() != null && ctx.key_parts != null && !ctx.key_parts.isEmpty()) {
      String key = ctx.key_parts.stream().map(Token::getText).collect(java.util.stream.Collectors.joining("")).trim();
  ...
  public void enterCrypto_map(Crypto_mapContext ctx) {
    ...
    if (seq != null) {
      FtdCryptoMapSet mapSet = _configuration.getCryptoMaps().computeIfAbsent(
          name, FtdCryptoMapSet::new);
      _currentCryptoMapEntry = mapSet.getEntries().computeIfAbsent(
          seq, s -> new FtdCryptoMapEntry(name, s));
  ...
  public void exitCm_match_address(Cm_match_addressContext ctx) {
    if (_currentCryptoMapEntry != null && ctx.acl_parts != null && !ctx.acl_parts.isEmpty()) {
      ...
      String aclName = ctx.acl_parts.stream()
          .map(token -> token.getText())
          .collect(java.util.stream.Collectors.joining(" "))
          .trim();
  ...
  public void enterObject_stanza(Object_stanzaContext ctx) {
    if (ctx.object_type().NETWORK() != null) {
      ...
      _currentNetworkObject = _configuration.getNetworkObjects().computeIfAbsent(
          name, FtdNetworkObject::new);
  ...
  public void enterObject_group_stanza(Object_group_stanzaContext ctx) {
    if (ctx.group_type().NETWORK() != null) {
      ...
      _currentNetworkObjectGroup = _configuration.getNetworkObjectGroups().computeIfAbsent(
          name, FtdNetworkObjectGroup::new);
      defineStructure(FtdStructureType.NETWORK_OBJECT_GROUP, name, ctx);
    } else if (ctx.group_type().SERVICE() != null) {
      String name = getObjectGroupName(ctx);
      _currentServiceObjectGroup = _configuration.getServiceObjectGroups().computeIfAbsent(
          name, FtdServiceObjectGroup::new);
  ...
  private FtdNatSource toNatSource(Nat_sourceContext ctx) {
    FtdNatSource.Type type = ctx.STATIC() != null ? FtdNatSource.Type.STATIC : FtdNatSource.Type.DYNAMIC;
  ...
  public void exitRoute_stanza(Route_stanzaContext ctx) {
    if (ctx.network != null && ctx.mask != null && ctx.gateway != null && ctx.interface_name() != null) {
  ...
  public void enterRouter_ospf_stanza(Router_ospf_stanzaContext ctx) {
    ...
    _currentOspfProcess = _configuration.getOspfProcesses().computeIfAbsent(pid, FtdOspfProcess::new);
  }
  ...
  @Override
  public void exitOspf_network(org.batfish.grammar.cisco_ftd.FtdParser.Ospf_networkContext ctx) {
  ...
  @Override
  public void exitOspf_router_id(org.batfish.grammar.cisco_ftd.FtdParser.Ospf_router_idContext ctx) {
  ...
  @Override
  public void exitOspf_passive_interface(org.batfish.grammar.cisco_ftd.FtdParser.Ospf_passive_interfaceContext ctx) {
  ...
  public void exitBgp_neighbor(Bgp_neighborContext ctx) {
    if (_currentBgpProcess != null && ctx.ip != null) {
      Ip ip = Ip.parse(ctx.ip.getText());
      FtdBgpNeighbor neighbor = _currentBgpProcess.getNeighbors().computeIfAbsent(ip, k -> new FtdBgpNeighbor(k));
  ...
  private FtdAccessListAddressSpecifier toAddressSpecifier(Acl_address_specContext ctx) {
    ...
    if (ctx.ip != null && ctx.mask != null) {
      return FtdAccessListAddressSpecifier.networkMask(Ip.parse(ctx.ip.getText()), Ip.parse(ctx.mask.getText()));
    }
    assert false;
    return null;
  }
