...
public class CompareSameNameAnswer extends Answer {
   ...
   private void processAccessPathLists(List<String> nodes,
      ...
      for (String node : nodes) {
         ...
         Map<String, AsPathAccessList> asPathAccessLists = configurations.get(
               node).getAsPathAccessLists();
         for (String asPathAccessListName : asPathAccessLists.keySet()) {
            ae.add(node, asPathAccessListName,
                  asPathAccessLists.get(asPathAccessListName));
         }
      }
      addAnswerElement(ae);
   }
   ...
   private void processCommunityLists(List<String> nodes,
      ...
      for (String node : nodes) {
         Map<String, CommunityList> communityLists = configurations.get(node)
               .getCommunityLists();
         for (String communityListName : communityLists.keySet()) {
            ae.add(node, communityListName,
                  communityLists.get(communityListName));
         }
      }
      addAnswerElement(ae);
   }
   ...
   private void processRouteFilterLists(List<String> nodes,
      ...
      for (String node : nodes) {
         ...
         Map<String, RouteFilterList> routeFilterLists = configurations.get(
               node).getRouteFilterLists();
         for (String routeFilterListName : routeFilterLists.keySet()) {
            ae.add(node, routeFilterListName,
                  routeFilterLists.get(routeFilterListName));
         }
      }
      addAnswerElement(ae);
   }
