...
public class TransformationEvaluatorTest {
  ...
  public void testShiftIpAddressIntoSubnet() {
    ...
    transformedFlow = eval(transformation, origFlow);
    assertThat(transformedFlow, equalTo(origFlow.toBuilder().setDstIp(new Ip("1.2.3.36")).build()));
  }
  ...
  public void testBranching() {
    ...
    Ip transformedSrcIp = new Ip("9.9.9.9");
    Transformation transformation =
    ...
    Ip origSrcIp = new Ip("8.8.8.8");
    _flowBuilder.setSrcIp(origSrcIp);

    Flow origFlow = _flowBuilder.setSrcIp(origSrcIp).setDstIp(new Ip("5.5.5.5")).build();
    Flow transformedFlow = eval(transformation, origFlow);
    assertThat(
