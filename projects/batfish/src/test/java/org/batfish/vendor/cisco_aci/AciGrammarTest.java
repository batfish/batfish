...
public class AciGrammarTest {
  ...
  /** Test contract with deny action filter that contains entries. */
  @Test
  public void testContractToAclConversion_denyActionWithEntries() {
    AciConfiguration config = new AciConfiguration();
    config.setHostname("test-fabric");
    ...
    filterRef.setName("deny_filter");
    filterRef.setAction("deny"); // The contract subject sets the action

    subject.setFilters(ImmutableList.of(filterRef));
    contract.setSubjects(ImmutableList.of(subject));

    // Define the actual filter with an entry
    org.batfish.vendor.cisco_aci.representation.FilterModel filter =
        new org.batfish.vendor.cisco_aci.representation.FilterModel("deny_filter");
    filter.setTenant("tenant1");
    org.batfish.vendor.cisco_aci.representation.FilterModel.Entry entry =
        new org.batfish.vendor.cisco_aci.representation.FilterModel.Entry();
    entry.setName("e1");
    entry.setProtocol("tcp"); // Should be denied
    filter.setEntries(ImmutableList.of(entry));

    config.getContracts().put("tenant1:deny_contract", contract);
    config.getFilters().put("tenant1:deny_filter", filter);
    config.finalizeStructures();
    ...
    // First line should be explicit deny from the entry
    assertThat(((ExprAclLine) acl.getLines().get(0)).getAction(), equalTo(LineAction.DENY));
    // Last line should be implicit deny
    assertThat(
        ((ExprAclLine) acl.getLines().get(acl.getLines().size() - 1)).getAction(),
        equalTo(LineAction.DENY));
