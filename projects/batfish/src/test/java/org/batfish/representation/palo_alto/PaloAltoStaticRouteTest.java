  ...
  private PaloAltoConfiguration parsePaloAltoConfig(String content) {
    ...
    PaloAltoControlPlaneExtractor extractor =
        new PaloAltoControlPlaneExtractor(
            content, parser, parseWarnings, new SilentSyntaxCollection());
  ...
  public void testStaticRouteWithNextHopIp() {
    String content =
        "set deviceconfig system hostname FW1\n"
            + "set network virtual-router VR1 routing-table ip static-route ROUTE-NAME nexthop"
            + " ip-address 10.10.10.10/24\n"
            + "set network virtual-router VR1 routing-table ip static-route ROUTE-NAME destination"
            + " 1.1.1.0/24\n";
