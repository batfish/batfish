...
public class RouteMapConvertorTest {
  ...
  public void testRouteMapExhaustive() throws IOException {
    ...
    // 20 permit tag 2, continue      OR    30 terminate
    {
      // false false -> 30 only
      policy.call(
              env.setOriginalRoute(base).setOutputRoute(outputBuilder).build());
    ...
    }
    {
      // false true -> 20, 30
      policy.call(
              env.setOriginalRoute(base.toBuilder().setTag(2L).build()).setOutputRoute(outputBuilder).build());
    ...
    }
    {
      ...
      policy.call(
              env.setOriginalRoute(base.toBuilder().setTag(1L).build()).setOutputRoute(outputBuilder).build());
