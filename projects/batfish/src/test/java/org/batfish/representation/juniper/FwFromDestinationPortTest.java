...
public class FwFromDestinationPortTest {
  private static void assertMatches(AclLineMatchExpr expr, Flow flow) {
    ...
    assertThat(
        trace.get(0).getTraceElement().getText(), containsString("Matched destination-port"));
  }
  ...
  public void testConversion() {
    AclLineMatchExpr converted =
        new FwFromDestinationPort(new SubRange(1, 2)).toAclLineMatchExpr(null, null, null);
    Flow testFlow =
        Flow.builder()
            .setIngressNode("c")
            .setSrcPort(5)
    ...
    assertMatches(converted, testFlow.toBuilder().setDstPort(2).build());
    assertNoMatches(converted, testFlow.toBuilder().setDstPort(5).build());
  }
}
