...
public final class FlatJuniperGrammarTest {
  ...
  public void testPsThenConflictsExtraction() {
    ...
    Set<PsThen> crossThens = pses.get("CROSS-ATTR").getTerms().get("t1").getThens().getAllThens();
  ...
  public void testPsThenConflictsConversion() {
    ...
    // FC-ACCEPT-REJECT: accept; reject → reject wins (last-wins among bare terminators)
    {
  ...
  public void testPsThenConflictsWarnings() {
    ...
    assertThat(
        ...
        hasItems(
            ...
            // accept; reject — last-wins among bare terminators
            hasComment("RISK: Overwriting existing then bare-terminator"),
            // accept; next term — prior accept suppressed at runtime by next term
            hasComment(
                "RISK: then next term suppresses prior then accept/reject in the same term:"
                    + " route falls through to the next term"),
            // next term; accept — bare accept after next-term is dead config
            hasComment(
                "RISK: then accept/reject has no effect when then next term is also present in"
                    + " the same term: route falls through to the next term")));
