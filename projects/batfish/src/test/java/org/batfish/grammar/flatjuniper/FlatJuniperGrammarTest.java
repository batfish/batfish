        ...
        hasItems(
            ...
            hasComment(
                "RISK: then next term/next policy suppresses prior then accept/reject in the"
                    + " same term: bare terminator does not fire"),
            // next term; accept — bare accept after next term/next policy is dead config
            hasComment(
                "RISK: then accept/reject has no effect when then next term/next policy is also"
                    + " present in the same term: bare terminator does not fire")));
