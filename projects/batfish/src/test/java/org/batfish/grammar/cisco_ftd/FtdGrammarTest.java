...
public class FtdGrammarTest {
  ...
  protected FtdConfiguration parseVendorConfig(String fileText) {
    ...
    FtdControlPlaneExtractor extractor =
        new FtdControlPlaneExtractor(fileText, parser, warnings, silentSyntax);
  ...
  public void testNatServiceSourceTranslationConversion() {
    String config =
        join(
            ...
            " host 192.0.2.20",
            "nat (inside,outside) source static REAL_SRC_SVC MAPPED_SRC_SVC service tcp 12345"
                + " 23456");
  ...
  public void testNatOrderingAcrossInterfacesConversion() {
    ...
    assertThat(
        c.getAllInterfaces().get("Ethernet0/2").getOutgoingTransformation(), equalTo(expectedDmz));
  }
  ...
  public void testNatDestinationAcrossInterfacesConversion() {
    ...
    Transformation expectedOutside =
        Transformation.when(
                AclLineMatchExprs.and(
                    AclLineMatchExprs.matchSrcInterface("Ethernet0/1"),
    ...
    assertThat(
        c.getAllInterfaces().get("Ethernet0/2").getIncomingTransformation(), equalTo(expectedDmz));
  }
  ...
  public void testServiceObjectGroupAclConversion() {
    ...
    assertThat(
        ...
        containsInAnyOrder(new SubRange(53, 53), new SubRange(80, 80), new SubRange(443, 443)));
  }
  ...
  public void testMpfParsing() {
    ...
    assertThat(
        vc.getClassMaps().get("CMAP1").getMatchLines(),
    ...
    assertThat(
        vc.getServicePolicies().get(1).getScope(), equalTo(FtdServicePolicy.Scope.INTERFACE));
  ...
  public void testMpfClassActionsParsing() {
    ...
    assertThat(
        ...
        containsInAnyOrder("inspect ftp", "set connection timeout half-closed 0:10:00"));
    assertThat(
        policyMap.getClassActionLines().get("CMAP2"),
        containsInAnyOrder("parameters", "timeout 1:00:00"));
  }
  ...
  public void testAccessListParsing() {
    String config =
        "access-list ACL_1 extended permit ip any any\n"
  ...
  public void testPrefilterAdvancedTrustParsing() {
    String config =
        join(
            ...
            "access-list PREFILTER_ACL advanced trust udp ifc OUTSIDE any any eq domain rule-id 101"
                + " event-log flow-end");
    FtdConfiguration vc = parseVendorConfig(config);
  ...
  public void testIpsecTransformSetParsing() throws IOException {
    ...
    assertThat(
        transformSet.getEspAuthentication(), equalTo(IpsecAuthenticationAlgorithm.HMAC_SHA1_96));
  }
  ...
  public void testVpnConversion() {
    ...
    Configuration c = vc.toVendorIndependentConfigurations().get(0);
    Interface vpnIface = c.getAllInterfaces().get("Ethernet0/0");
    ...
    assertThat(
        c.getIpsecPeerConfigs().keySet(), contains("~IPSEC_PEER_CONFIG:MYMAP:10:Ethernet0/0~"));
  }
  ...
  public void testNatCombinedParsing() {
    String config =
        "nat (inside,outside) source static real_src mapped_src destination static real_dest"
            + " mapped_dest service http https\n";
    FtdConfiguration vc = parseVendorConfig(config);
