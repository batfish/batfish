...
/** Tests for {@link NxosRoutingTableExtractor}. */
public final class NxosRoutingTableExtractorTest {
  ...
  private RoutesByVrf testNextHop_RoutesByVrf() throws IOException {
    ...
    GrammarSettings settings = new MockGrammarSettings(true, 0, 0, 0, false, false, true, true, false);
    Warnings warnings = new Warnings();

    String showIpRoute =
        CommonUtil.readResource("org/batfish/grammar/routing_table/nxos/test_next_hop.txt");

    NxosRoutingTableCombinedParser parser =
        new NxosRoutingTableCombinedParser(showIpRoute, settings);
    NxosRoutingTableExtractor extractor =
        new NxosRoutingTableExtractor("hostname", showIpRoute, parser, warnings, batfish);
