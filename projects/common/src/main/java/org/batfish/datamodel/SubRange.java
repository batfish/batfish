...
public final class SubRange implements Serializable, Comparable<SubRange> {
  ...
  public SubRange(@Nullable Object o) {
    if (o instanceof String) {
      ...
      if (parts.length != 2) {
        throw new IllegalArgumentException("Invalid subrange: \"" + s + "\"");
      }
      try {
        _start = Integer.parseInt(parts[0]);
      } catch (NumberFormatException e) {
        throw new IllegalArgumentException("Invalid subrange start: \"" + parts[0] + "\"", e);
      }
      try {
        _end = Integer.parseInt(parts[1]);
      } catch (NumberFormatException e) {
        throw new IllegalArgumentException("Invalid subrange end: \"" + parts[1] + "\"", e);
      }
      ...
      throw new IllegalArgumentException("Cannot create SubRange from null");
    } else {
