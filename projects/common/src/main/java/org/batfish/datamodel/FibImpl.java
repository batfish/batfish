...
public final class FibImpl implements Fib {
  ...
  private @Nonnull Map<Prefix, IpSpace> computeNonTrivialMatchingIps() {
     ...
     * to the builder) and return the prefixes matched by any prefix in that subtrie. To create
     * the matching Ips of the prefix, whitelist the prefix and blacklist the IPs matched by
     * subtrie prefixes (i.e. longer prefixes).
     *
     * Subtrie prefixes are passed up as lists, not sets: the two subtries of a node are disjoint,
     * so concatenation is exact, and hashing into a set is only paid at the nodes that need one.
     */
    _root.fold(
        new FoldOperator<FibEntry, List<IpWildcard>>() {
          @Override
          public @Nonnull List<IpWildcard> fold(
              Prefix prefix,
              Set<FibEntry> elems,
              @Nullable List<IpWildcard> leftPrefixes,
              @Nullable List<IpWildcard> rightPrefixes) {
            List<IpWildcard> subTriePrefixes;
            boolean leftEmpty = leftPrefixes == null || leftPrefixes.isEmpty();
            boolean rightEmpty = rightPrefixes == null || rightPrefixes.isEmpty();
            if (leftEmpty && rightEmpty) {
              subTriePrefixes = ImmutableList.of();
            } else if (leftEmpty) {
              subTriePrefixes = rightPrefixes;
            } else if (rightEmpty) {
              subTriePrefixes = leftPrefixes;
            } else {
              List<IpWildcard> both = new ArrayList<>(leftPrefixes.size() + rightPrefixes.size());
              both.addAll(leftPrefixes);
              both.addAll(rightPrefixes);
              subTriePrefixes = both;
            }
            ...
            if (!subTriePrefixes.isEmpty()) {
              ...
              builder.put(
                  ...
                  IpWildcardSetIpSpace.create(
                      ImmutableSet.copyOf(subTriePrefixes), ImmutableSet.of(wc)));
            }

            return ImmutableList.of(wc);
          }
        });
