...
import org.batfish.specifier.parse.grammar.SpecifierLexer;
import org.batfish.specifier.parse.grammar.SpecifierParser;
 ...
 *
 * <p>The output is consumed by {@link ParsedAutoComplete}, which is unchanged: it switches on each
 * match's {@link Anchor.Type} to produce suggestions. Two kinds of matches are produced, mirroring
 * parboiled:
 *
 * <ul>
 ...
 *       the enclosing rule stack, which {@link ParsedAutoComplete} walks to find the ancestor
...
final class C3PotentialMatches {
   ...
   * so that {@link ParsedAutoComplete#findPrecedingInput} can recover input spans.
   */
