...
public final class ParsedFilterSpecifier implements FilterSpecifier {
  ...
  private final class FilterAstNodeToFilters implements FilterAstNodeVisitor<Set<IpAccessList>> {
    ...
    public Set<IpAccessList> visitFilterWithNodeFilterAstNode(
      ...
      return new ParsedNodeSpecifier(filterWithNodeFilterAstNode.getNodeAstNode())
          .resolve(_ctxt).stream()
              ...
              .flatMap(
                  n ->
                      (new ParsedFilterSpecifier(filterWithNodeFilterAstNode.getFilterAstNode())
                              .resolve(n, _ctxt))
                          .stream())
    ...
    public @Nonnull Set<IpAccessList> visitInFilterAstNode(InFilterAstNode inFilterAstNode) {
      return new InterfaceSpecifierFilterSpecifier(
              ...
              new ParsedInterfaceSpecifier(inFilterAstNode.getInterfaceAst()))
          .resolve(_node, _ctxt);
    }
  ...
  ParsedFilterSpecifier(FilterAstNode ast) {
    _ast = ast;
  }
   ...
   */
  public static ParsedFilterSpecifier parse(String input) {
    AstNode ast = SpecifierAstBuilder.getAst(Grammar.FILTER_SPECIFIER, input);
    checkArgument(
        ast instanceof FilterAstNode, "ParsedFilterSpecifier requires an FilterSpecifier input");

    return new ParsedFilterSpecifier((FilterAstNode) ast);
  }
  ...
  public boolean equals(Object o) {
    ...
    if (!(o instanceof ParsedFilterSpecifier)) {
      return false;
    }
    return Objects.equals(_ast, ((ParsedFilterSpecifier) o)._ast);
  }
