...
import static org.batfish.specifier.parse.Anchor.Type.REFERENCE_BOOK_NAME;
...
/**
 * Tests for reference book entity auto completion in {@link ParsedAutoComplete}. The testing
 ...
 * </ol>
 */
public class ParsedAutoCompleteReferenceBookEntityTest {
  ...
  public void testAutoCompleteReferenceBookEntity() {
    ...
    assertThat(
        getPAC()
            .autoCompleteReferenceBookEntity(pm, "(b1a", ParsedAutoComplete::addressGroupGetter),
        containsInAnyOrder(
            new ParsedAutoCompleteSuggestion("g11", 42, ADDRESS_GROUP_NAME),
            new ParsedAutoCompleteSuggestion("g12", 42, ADDRESS_GROUP_NAME)));
  ...
  public void testAutoCompleteNodeRoleNamePrefix() {
    ...
    assertThat(
        pac.autoCompleteReferenceBookEntity(
            ...
            "(b1a",
            ParsedAutoComplete::addressGroupGetter),
        containsInAnyOrder(new ParsedAutoCompleteSuggestion("g11", 42, ADDRESS_GROUP_NAME)));
  ...
  public void testE2eNoTailMatchingBook() {
    ...
    assertThat(
        getPAC(query, Grammar.IP_SPACE_SPECIFIER).run(),
        containsInAnyOrder(
            new ParsedAutoCompleteSuggestion("b1a", 14, REFERENCE_BOOK_NAME),
            new ParsedAutoCompleteSuggestion("b2a", 14, REFERENCE_BOOK_NAME),
            new ParsedAutoCompleteSuggestion(
                ",", query.length(), REFERENCE_BOOK_AND_ADDRESS_GROUP_TAIL)));
  }
  ...
  public void testE2eNoGroupNamePrefix() {
    ...
    Set<ParsedAutoCompleteSuggestion> expected =
        ImmutableSet.of(
            new ParsedAutoCompleteSuggestion("g11", query.length(), ADDRESS_GROUP_NAME),
            new ParsedAutoCompleteSuggestion("g12", query.length(), ADDRESS_GROUP_NAME));
  ...
  public void testE2eGroupNamePrefix() {
    ...
    Set<ParsedAutoCompleteSuggestion> expected =
        ImmutableSet.of(
            new ParsedAutoCompleteSuggestion("g11", 18, ADDRESS_GROUP_NAME),
  ...
  public void testE2eLocationFuncGroupNamePrefix() {
    ...
    assertThat(
        ...
        containsInAnyOrder(
            new ParsedAutoCompleteSuggestion("g11", 31, ADDRESS_GROUP_NAME),
            new ParsedAutoCompleteSuggestion(")", query.length(), OPERATOR_END)));
  }
  ...
  public void testE2eSpacesGroupNamePrefix() {
    ...
    assertThat(
        getPAC(query, Grammar.IP_SPACE_SPECIFIER).run(),
        containsInAnyOrder(
            new ParsedAutoCompleteSuggestion("g11", 23, ADDRESS_GROUP_NAME),
            new ParsedAutoCompleteSuggestion(")", query.length(), OPERATOR_END)));
  }
  ...
  public void testE2eInterfaceGroup() {
    ...
    assertThat(
        getPAC(query, Grammar.INTERFACE_SPECIFIER).run(),
        containsInAnyOrder(
            new ParsedAutoCompleteSuggestion("i11", query.length(), INTERFACE_GROUP_NAME),
            new ParsedAutoCompleteSuggestion("i12", query.length(), INTERFACE_GROUP_NAME)));
  }
}
