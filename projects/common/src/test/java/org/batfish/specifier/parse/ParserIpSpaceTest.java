...
import static org.batfish.specifier.parse.Anchor.Type.IP_ADDRESS;
import static org.batfish.specifier.parse.Anchor.Type.IP_PREFIX;
import static org.batfish.specifier.parse.Anchor.Type.IP_RANGE;
import static org.batfish.specifier.parse.Anchor.Type.IP_SPACE_SET_OP;
import static org.batfish.specifier.parse.Anchor.Type.IP_WILDCARD;
...
public class ParserIpSpaceTest {
  ...
  public void testIpCompletion() {
    ...
    assertThat(
        suggestions,
        containsInAnyOrder(
            new ParsedAutoCompleteSuggestion("1.1.1.1", 0, IP_ADDRESS),
            new ParsedAutoCompleteSuggestion("1.1.1.10", 0, IP_ADDRESS),
            new ParsedAutoCompleteSuggestion("-", query.length(), IP_RANGE),
            new ParsedAutoCompleteSuggestion(":", query.length(), IP_WILDCARD),
            new ParsedAutoCompleteSuggestion("1.1.1.1/22", 0, IP_PREFIX),
            new ParsedAutoCompleteSuggestion("\\", query.length(), IP_SPACE_SET_OP),
            new ParsedAutoCompleteSuggestion("&", query.length(), IP_SPACE_SET_OP),
            new ParsedAutoCompleteSuggestion(",", query.length(), IP_SPACE_SET_OP)));
  }
  ...
  public void testContextSensitiveAddressGroup() {
    ...
    assertThat(
        ...
        containsInAnyOrder(
            new ParsedAutoCompleteSuggestion("g1", query.length(), ADDRESS_GROUP_NAME)));
  }
}
