...
public class ParsedInterfaceSpecifierTest {
  ...
  public void testResolveInterfaceGroup() {
    ...
    build();
    assertThat(
        new ParsedInterfaceSpecifier(new InterfaceGroupInterfaceAstNode(book, interfaceGroup))
            .resolve(_nodes, _ctxt),
        contains(NodeInterfacePair.of(_iface11)));

    // reverse is also the same
    assertThat(
        new ParsedInterfaceSpecifier(new InterfaceGroupInterfaceAstNode(interfaceGroup, book))
            .resolve(_nodes, _ctxt),
        contains(NodeInterfacePair.of(_iface11)));
  }
  ...
  public void testResolveName() {
    build();
    assertThat(
        new ParsedInterfaceSpecifier(new NameInterfaceAstNode("iface11")).resolve(_nodes, _ctxt),
        contains(NodeInterfacePair.of(_iface11)));
    // This regex looking name below should not match anything
    assertThat(
        new ParsedInterfaceSpecifier(new NameInterfaceAstNode("iface1.*")).resolve(_nodes, _ctxt),
        empty());
  }
  ...
  public void testResolveNameRegex() {
    build();
    assertThat(
        new ParsedInterfaceSpecifier(new NameRegexInterfaceAstNode("iface.*2"))
            .resolve(_nodes, _ctxt),
  ...
  public void testResolveType() {
    ...
    build();
    assertThat(
        new ParsedInterfaceSpecifier(new TypeInterfaceAstNode("tunnel")).resolve(_nodes, _ctxt),
        contains(NodeInterfacePair.of(_iface12)));
  }
  ...
  public void testResolveVrf() {
    ...
    assertThat(
        new ParsedInterfaceSpecifier(new VrfInterfaceAstNode("vrf1")).resolve(_nodes, _ctxt),
        contains(NodeInterfacePair.of(_iface11)));
  }
  ...
  public void testResolveZone() {
    ...
    build();
    assertThat(
        new ParsedInterfaceSpecifier(new ZoneInterfaceAstNode("zone1")).resolve(_nodes, _ctxt),
        containsInAnyOrder(NodeInterfacePair.of(_iface11), NodeInterfacePair.of(_iface2)));
  }
  ...
  public void testParseBadInput() {
    ...
    ParsedInterfaceSpecifier.parse("@connected");
  }

  @Test
  public void testParseGoodInput() {
    assertThat(
        ParsedInterfaceSpecifier.parse("eth0"),
        equalTo(new ParsedInterfaceSpecifier(new NameInterfaceAstNode("eth0"))));
  }
}
