...
import static org.batfish.specifier.parse.Anchor.Type.UNKNOWN;
import static org.batfish.specifier.parse.ParsedAutoComplete.updateSuggestions;
...
/** Tests for {@link ParsedAutoComplete} */
public class ParsedAutoCompleteTest {

  @Rule public ExpectedException _thrown = ExpectedException.none();

  // These helpers build a ParsedAutoComplete over the node specifier grammar. They are used only
  ...
  private static ParsedAutoComplete getTestPAC(String query) {
    return getTestPAC(query, CompletionMetadata.builder().build());
  }

  private static ParsedAutoComplete getTestPAC(
  ...
  public void testAutoCompletePotentialMatchStringLiteral() {
    ...
    assertThat(
        ...
        containsInAnyOrder(new ParsedAutoCompleteSuggestion("pfxcomp", 0, UNKNOWN)));
  }
  ...
  public void testFindFirstMatchingPathElement() {
    ...
    assertThat(
        ParsedAutoComplete.findFirstMatchingPathElement(pm, 2, NODE_AND_INTERFACE_TAIL),
        equalTo(Optional.of(tailStart)));

    // non-existent anchor type
    assertThat(
        ParsedAutoComplete.findFirstMatchingPathElement(pm, 2, NODE_NAME),
        equalTo(Optional.empty()));

    // should return the other interface_name
    assertThat(
        ParsedAutoComplete.findFirstMatchingPathElement(pm, 2, INTERFACE_NAME),
  ...
  public void testFindPrecedingInputMissingAnchor() {
    ...
    ParsedAutoComplete.findPrecedingInput(
        new PotentialMatch(
  ...
  public void testFindPrecedingInputMissingTail() {
    ...
    _thrown.expect(IllegalArgumentException.class);
    ParsedAutoComplete.findPrecedingInput(pm, "dummy", NODE_AND_INTERFACE, NODE_AND_INTERFACE_TAIL);
  }
  ...
  public void testCompleteNodeNameWithHumanNameHint() {
    ...
    assertThat(
        ...
        allOf(
            ...
            hasItem(new ParsedAutoCompleteSuggestion(node2, NODE_NAME.getHint(), 0, NODE_NAME))));
  }
  ...
  public void testCompleteHumanName() {
    ...
    assertThat(
        getTestPAC(query, metadata).run(),
        allOf(
            hasItem(new ParsedAutoCompleteSuggestion(node1, null, 0, NODE_NAME, node1HumanName))));
  }

  /**
   * Tests that relevants fields are preserved when we covert {@link AutocompleteSuggestion} to
   * {@link ParsedAutoCompleteSuggestion}.
   */
  @Test
  public void testUpdateSuggestions() {
    ...
    assertThat(
        ...
        equalTo(
            ImmutableSet.of(
                new ParsedAutoCompleteSuggestion("text", "hint", 0, NODE_NAME, "desc"))));
  }
}
