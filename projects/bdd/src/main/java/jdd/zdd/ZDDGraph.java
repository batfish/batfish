...
public class ZDDGraph extends ZDD {
  ...
  /**
   * all pairs, that is, all possible edges in a fully connected graph. if V = { x_from, ..., x_to}
   * then allEdge(V) = { (v1,v2) | v1,v2 \in V. v1 != v2 }
   */
  public int allEdge() {
  ...
  /**
   * MaxSet(X) = { x \in X | \forall x' \in X. x \subseteq x' ==> x = x' } that is: MaxSet(X) = { x
   * \in X | \lnot \exist x' \in X - x. x \subseteq x' }
   */
