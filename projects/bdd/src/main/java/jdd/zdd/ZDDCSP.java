...
/**
 * ZDD operations for CSP problems. based on "On the properties of combination set operations", by
 * Okuno, Minato and Isozaki.
 *
 * <p>I am also thankfull to Paolo Bonzini who suggested replaceing the slow Exclude() with
 * noSubset() :)
 */
public class ZDDCSP extends ZDD2 {
  ...
  // ---------------------------------------------------------------
  /** slow Exclude conputed using the definition: Exclude(F,C) = F - Restrict(F,C) */
   ...
   *
   * <p>noSupset(F, C) = {f \in F | \lnot \exist c \in C, c \subseteq f }
   *
   * <p>NOTE THAT THE SAME CODE IS ALSO FOUND IN ZDDGraph.noSupset(F,C) We could have used some OOP
   * tricks to reuse that code here, but I decided not to...
   */
