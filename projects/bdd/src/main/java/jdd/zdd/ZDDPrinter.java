...
public class ZDDPrinter {
  ...
  /**
   * Print the Z-BDD as a node-table. You should call ZDD.print() instead of calling this function
   * directly
   */
  ...
  // ------------------------------------------------------------------------------------
  /** create a DOT file visualizing a Z-BDD tree. You should call ZDD.printDot() instead */
    ...
    try {
      ...
      if (had_0)
        ps.println(
            "\t0 [shape=box, label=\""
                + nn.zeroShort()
                + "\", style=filled, height=0.3, width=0.3];");
      if (had_1)
        ps.println(
            "\t1 [shape=box, label=\""
                + nn.oneShort()
