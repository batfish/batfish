...
/**
 * Base implementation for Zero-Suppressed Binary Decision Diagrams (Z-BDDs). Z-BDDs are a special
 * type of BDDs that are more suited for sparse sets. For example, if you are working with a set
 * that is most of the time empty, you might want to use Z-BDDs instead of BDDs.
 *
 * <p>The base implementation will give you the core Z-BDD operation such as change, diff and union.
 * To get more operators, see the sub-classes.
 *
 * @see BDD
 * @see ZDD2
 * @see ZDDCSP
...
public class ZDD extends NodeTable {
  ...
  /** number of Z-BDD variables */
  private int node_count_int;

  /** internal variable used by nodeCount */
   ...
   *
   * <p>This function was added to ease fast creation of sets of sets in tests.
   *
   * <p>E.g. "0001 1000 1101" => { v1, v4, v4v3v1 }
   */
  ...
  public final int intersect(int p, int q) {
    ...
    if (p == 1)
      return follow_low(
          q); // ADDED BY ARASH, NOT TESTED VERY MUCH YET, DONT NOW IF IT MAKES THINGS FASTER OR
              // SLOWER!!
  ...
  // --- [ satOne ] ----------------------------------------------
  /**
   * Returns a satisfying boolean assignment, null if none<br>
  ...
  public void print(int zdd) {
    ZDDPrinter.print(zdd, this, nodeNames);
  }

  public void printDot(String fil, int bdd) {
    ZDDPrinter.printDot(fil, bdd, this, nodeNames);
  }

  public void printSet(int bdd) {
    ZDDPrinter.printSet(bdd, this, nodeNames);
  }
