...
/**
 * some common operations involving arrays has been gathered here. The idea is to concenterate all
 * loops in the same class so it can be optimized by the JVM (loop unrolloing, SIMD etc).
 *
 * <p>we used to have all the loops unrolled here, but it turns out that the latest JVMs (1.4.2_02
 * and above) do a much better job.
 */
public final class Array {

  // [resize]------------------------------------------------
  /**
   * this is an implementation of realloc() for integers. the <tt>old_size</tt> old members are
  ...
  /**
   * this is an implementation of realloc() for short integers. the <tt>old_size</tt> old members
   ...
   *
   * <p>BEWRAE: stupid System.arraycopy() cant hande overlapping arrays correctly
   */
  ...
  // ----------------------------------------------------------------------------------------
  /**
   * unrolled code for copying in an array of integers where the source and destination may/may not
   * overlap without creating any problems
   */
  private static final void fast_copy_forward(int[] y, int o1, int[] x, int o2, int len) {
    for (int i = 0; i < len; i++) x[o2 + i] = y[o1 + i];
  }
  ...
  /**
   * unrolled code for copying in an array of short integers, where the source and destination
   * overlaps. to avoid overwriting the yet-to-be-copied members, we have to do the copy in backward
   * direction.
   */
