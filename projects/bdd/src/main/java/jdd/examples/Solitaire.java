...
/**
 * This is the solitare.cxx example copied from BuDDy source distribution convert to Java and JDD:
 *
 * <p><b>Note:</b> you <u>can not</u> run this example with the default JVM parameters (8 million
 * nodes would require at least 256 MB RAM)!!
 *
 * <p>Jorn Lind-Nielsen Included this in BuDDy as very good pedagogic example on sequential models.
...
public class Solitaire extends ProfiledBDD2 {
  ...
  private void make_initial_state() {
    I = 1;
    for (int n = 0; n < SIZE; n++) I = andTo(I, (n == CENTER) ? not_boardC[n] : boardC[n]);
    // printSet(I);
  }
  ...
  private int make_move(int src, int tmp, int dst) {
    ...
    // bdd move = boardC[src] & boardC[tmp] & !boardC[dst] & !boardN[src] & !boardN[tmp] &
    // boardN[dst];
  ...
  private void make_transition_relation() {
    ...
    JDDConsole.out.println(
        "Transition relation: "
            ...
            + " nodes, "
            + satCount(T)
            + " distinct transitions."); // XXX: this differs from orifiganl solitare.cxx!
  }
    ...
    do {
      ...
      JDDConsole.out.println(
          ""
              ...
              + nodeCount(reachable)
              + " nodes, "
              + (satCount(reachable) / dummyStateNum)
              + " states.");
