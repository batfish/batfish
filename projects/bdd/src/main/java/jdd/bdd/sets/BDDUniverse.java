...
/* package */ class SubDomain {
  ...
  /* package */ SubDomain(BDDUniverse universe, int size) {
    ...
    for (int i = 0; i < bits; i++) vars[i] = universe.createVar();
  ...
  public int find(int bdd) {
    ...
    for (int i = 0; i < size; i++) if (universe.and(bdd, numbers[i]) == bdd) return i;
...
public class BDDUniverse extends BDD implements Universe {
  ...
  */
  ...
  public int cardinality(int[] x) {
    int ret = 1;
    for (int i = 0; i < num_subdomains; i++) if (x[i] == -1) ret *= subdomains[i].getSize();
    return ret;
  }
