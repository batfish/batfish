...
public final class OptimizedCache extends SimpleCache {
  ...
  /**
   * possible_bins_count is the number of possible (at most) entries in the table. if that number is
   * zero, we wont need to clean the cache
   */
  ...
  /** If this function returns true, we should wipe the cache entirely instead of a partial clean */
  ...
  private final int partial_clean3(NodeTable nt) {
    ...
    for (int i = cache_size; i != 0; ) {
      i--;
      if (!isValid(i)
          || !nt.isValid(getIn(i, 1))
          || !nt.isValid(getIn(i, 2))
          || !nt.isValid(getIn(i, 3))
          || !nt.isValid(getOut(i))) invalidate(i);
      else ok++;
    }
    return ok;
  }
  ...
  private final int partial_clean1(NodeTable nt) {
    ...
    for (int i = cache_size; i != 0; ) {
      i--;
      if (!isValid(i) || !nt.isValid(getIn(i, 1)) || !nt.isValid(getOut(i))) invalidate(i);
      else ok++;
    }
    return ok;
  }
