...
public class BDD extends NodeTable {
  ...
  /**
   * permutations are gathered in a linked list. this is to avoid having multiple objects for same
   * permutation.
   */
  ...
  /**
   * create a BDD manager with initially <tt>nodesize</tt> nodes and <tt>cache_size</tt> cache
   * elements.
   */
  public BDD(int nodesize, int cache_size) {
    ...
    relprod_cache =
        new OptimizedCache("REL-PROD", cache_size / Configuration.bddRelprodcacheDiv, 3, 3);

    // MAY GROW. (bdd1, perm-id) => bdd
    replace_cache =
        new OptimizedCache("REPLACE", cache_size / Configuration.bddReplacecacheDiv, 2, 1);
  ...
  public int createVar() {
    ...
    if (support_buffer.length < num_vars) support_buffer = new boolean[num_vars * 3];
  ...
  /**
   * this function is called after the garbage collector hash changed some internal data. when we
   * get here, data structures such as caches are instable, so we have to clean up after the garbage
   * collector.
   */
   ...
   *
   * <p>given a list of variables <i>v[]</i>, it vill return <i>v[0] AND v[1] AND ...</i>. this cube
   * is then used in operations such as exists and forall
   *
   * @see #exists
   * @see #forall
   */
  ...
  /**
   * a cube over a set of variables represented as a mintem. for example the string "11-1" over four
   * variables will return the cube <tt>v1 AND v3 AND v4</tt>.
   *
   * @see #cube
   */
  ...
  /**
   * returns a unary minterm based on a vector of boolean assignments. for example <i>minterm([true,
   * false]) </i> will return <i>NOT v1 and v2</i>.
   *
   * @see #minterm
   */
   ...
   *
   * <p>it can be used to (inefficiently) simulate and binary operation. (i think it is described in
   * the "Long" paper).
   *
  ...
  public int ite(int f, int then_, int else_) {
    ...
    if ((getLow(f) < 2 && getHigh(f) < 2)
        && (getVar(f) < getVar(then_))
        && (getVar(f) < getVar(else_))) {
  ...
  /**
   * internal ITE recursive function. note that are ITE terminal cases differs from BuDDy since our
   * 0/1 constant variables differs
   */
  private final int ite_rec(int f, int g, int h) {
    ...
    int l =
        nstack.push(
            ite_rec(
                (v == getVar(f)) ? getLow(f) : f,
                (v == getVar(g)) ? getLow(g) : g,
                (v == getVar(h)) ? getLow(h) : h));

    int H =
        nstack.push(
            ite_rec(
                (v == getVar(f)) ? getHigh(f) : f,
                (v == getVar(g)) ? getHigh(g) : g,
                (v == getVar(h)) ? getHigh(h) : h));
   ...
   *
   * <p>Let <i>exists(bdd, variable)</i> to be <i>bdd(variable) OR bdd(NOT variable)</i>. Then
   * <i>exists(bdd, cube)</i> does this for every (combination of) variable in the cube.
   *
   * <p>Note that all references of <i>variable</i> and the variables in <i>cube</i> will be removed
   ...
   *
   * <p>Let <i>forall(bdd, variable)</i> to be <i>bdd(variable) AND bdd(NOT variable)</i>. Then
   * <i>foral(bdd, cube)</i> does this for every (combination of) variable in the cube.
   *
   * <p>Note that all references of <i>variable</i> and the variables in <i>cube</i> will be removed
   * from the resulting bdd.
   *
   ...
   *
   * <p>It combines a conjunction and existential quantification that is often seen in image
   * computation. Use this operation for better runtime performance!
   */
  ...
  /**
   * Given a permutation, this function will re-label (some) nodes in a BDD to associate with a new
   * sets of variables.
   *
   * @see Permutation
  ...
  // ----[ restrict ] -----------------------------
  /**
   * The BDD restrict operation. Tries to minimize the size of a BDD by using dont-cares. For more
   * details, check out <i>"A unified framework for the formal verification of circuits</i>, by
   * Coudert and Madre.
   *
   * @see #simplify
   */
   ...
   *
   * <p>XXX: we have no cache for it yet!
   *
   * @see #restrict
   */
  ...
  /**
   * returns the number of satisfying assignments for this BDD. beware of possible floating-point
   * overflow here!
   *
   * <p>Important note: this works because getVar() returns "numbers of variables plus one" for the
   * terminal nodes!
   */
  ...
  protected double satCount_rec(int bdd) {
    ...
    double ret =
        (satCount_rec(low) * Math.pow(2, getVar(low) - getVar(bdd) - 1))
  ...
  // ---- [oneSat ] -----------------------------------
  /**
   * return a satisfying assignment for this BDD as a unary cube. bdd may not be the constant ONE or
   * ZERO.
   */
  ...
  /**
   * oneSat(bdd, buffer) returns an int vector x where x[i] is 0/1 or -1 for neg cofactor/pos
   * cofactor and dont care
   *
  ...
  // ---- [ support ] ---------------------------------------------
  /**
   * returns the support set of a variable, that is a cube C where each of its variables are used
   * somewhere in the ibpu bdd.
   */
   ...
   *
   * <p><b>Note:</b> This method is far far more efficient than computing the "minterm" as BDD, then
   * OR it to "bdd" and see if the result is not equal to "bdd"!
   */
  public boolean member(int bdd, boolean[] minterm) {

    while (bdd >= 2) bdd = (minterm[getVar(bdd)]) ? getHigh(bdd) : getLow(bdd);
