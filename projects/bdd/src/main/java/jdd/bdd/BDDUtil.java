...
public class BDDUtil {
  ...
  /**
   * encode the number as a BDD. must use the same encoding as numberToMinterm!
   *
   * @see #numberToMinterm
   */
  ...
  /**
   * encode the number as a boolean vector, starting from the given index. must use the same
   * encoding as numberToBDD!
   *
   * @see #numberToBDD
   */
  public static void numberToMinterm(int num, int length, int index, boolean[] output) {
    for (int i = 0; i < length; i++) output[index++] = ((num & (1L << i)) != 0);
  }
}
