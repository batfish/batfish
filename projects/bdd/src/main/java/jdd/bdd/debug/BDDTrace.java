 ...
 *
 * <p>It reads trace files a subset of Bwolen Yang's "BDD Trace Driver" file format. It is used to
 * verify the integrity and efficiency of a BDD package against another without creating a whole new
 * application for it...
 *
 * <p>Note: we have added the following commands:<br>
 * <b>print_bdd(variable)</b> print the BDD strcutre in the java console. <b>show_bdd(variable)</b>
 * save an image for this BDD in current directory (default is variable.png).
 * <b>save_bdd(variable)</b> save this BDD in the current directory.
 *
 * <p>If you are using an applet, only <tt>print_bdd</tt> will work. <br>
 */
public class BDDTrace {
  ...
  class TracedBDDOperation extends TracedOperation {
      ...
      if (size != -1) {
        ...
        if (size != size2) {
          JDDConsole.out.println(
              "\n*************************************************************************");
          JDDConsole.out.println(
              "Size comparison failed after " + op + " ( wanted " + size + ", got " + size2 + ")");
          show();
          JDDConsole.out.println("\n");
    ...
    private void do_or() {
      if (ops == 2) ret.bdd = bdd.or(op1.bdd, op2.bdd);
        ...
        for (Enumeration e = operands.elements(); e.hasMoreElements(); )
          if (((TracedVariable) e.nextElement()).bdd == 1) {
            ret.bdd = 1;
            return;
          }
    ...
    private void do_and() {
      if (ops == 2) ret.bdd = bdd.and(op1.bdd, op2.bdd);
        ...
        for (Enumeration e = operands.elements(); e.hasMoreElements(); )
          if (((TracedVariable) e.nextElement()).bdd == 0) {
            ret.bdd = 0;
            return;
          }
    ...
    public void show_code() {
      ...
      if (op.equals("ite"))
        JDDConsole.out.println(
            "System.out.println(\"" + ret.name + " ==> \"+" + ret.name + ".nodeCount());");
    }
  }
  ...
  public BDDTrace(String file, InputStream is, int nodes) throws IOException {
    ...
    Options.verbose = false; // to avoid GC messaages
    execute();
  ...
  private void checkVar(TracedVariable v) {
    if (v != null && v.last_use == op_count) {
      // JDDConsole.out.println("Removing " + v.name + " at state " + op_count + ", bdd = " +
      // v.bdd);
      bdd.deref(v.bdd);
  ...
  private void show_results() {
    ...
    JDDConsole.out.println(
        "" + op_count + " operations performed, total execution time: " + time + " [ms]");

    if (Options.verbose) {
      if (last_assignment != null) {
        int size = node_count(last_assignment);
        JDDConsole.out.println(
            "Last assginment: " + last_assignment.name + ", " + size + " nodes.");
  ...
  private void read_structure() throws IOException {
    ...
    while (true) {
      ...
      if (ret.equals("trace_verbose_print")) {
        ...
        need("(");
        String str = need();
        TracedVariable t1 = needVar(str);
        need(",");
        str = need();
        TracedVariable t2 = needVar(str);
        need(")");
        need(";");
