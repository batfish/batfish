...
public class DebugBDD extends BDD {
  ...
  private void check_node(int bdd, String msg) {
    ...
    if (getRef(bdd) <= 0) err = "Unrefrenced node , '" + msg + "'";
    else err = ntc.checkNode(bdd, msg);
    if (err != null) {
  ...
  protected void post_removal_callbak() {
    ...
    if (!quant_cache.check_cache(this)) fatal(null, "quant_cache sanity check failed");

    if (!replace_cache.check_cache(this)) fatal(null, "replace_cache sanity check failed");

    if (!not_cache.check_cache(this)) fatal(null, "not_cache sanity check failed");

    if (!op_cache.check_cache(this)) fatal(null, "op_cache sanity check failed");
  }
  // --------------------------------------------
  /*
