...
/**
 * An implementation of BDDFactory that relies on the CUDD library through a native interface. You
 * can use this by calling the "CUDDFactory.init()" method with the desired arguments. This will
 * return you an instance of the BDDFactory class that you can use. Call "done()" on that instance
 * when you are finished.
 *
 * <p>CUDD does not have much of the functionality that BuDDy has, and it has not been well-tested.
 * Furthermore, it is slower than BuDDy. Therefore, it is recommended that you use the BuDDy library
 * instead.
 *
 * <p>This class (and the CUDD library) do NOT support multithreading. Furthermore, there can be
 * only one instance active at a time. You can only call "init()" again after you have called
 * "done()" on the original instance. It is not recommended to call "init()" again after calling
 * "done()" unless you are _completely_ sure that all BDD objects that reference the old factory
 * have been freed.
 *
 * <p>If you really need multiple BDD factories, consider using the JavaFactory class for the
 * additional BDD factories --- JavaFactory can have multiple factory instances active at a time.
 *
