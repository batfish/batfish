...
public class JDDFactory extends BDDFactory {
  ...
  public BDD ithVar(int var) {
    if (var >= bdd.numberOfVariables()) throw new BDDException();
  ...
  public void setVarOrder(int[] neworder) {
    ...
    if (var2level != null) throw new BDDException();

    if (bdd.numberOfVariables() != neworder.length) throw new BDDException();
    ...
    for (int i = 0; i < bdd.numberOfVariables(); ++i) {
      int k = neworder[i];
      // System.out.println("Var "+k+" (node "+vars[k]+") in original order -> var "+i+" (node
      // "+vars[i]+") in new order");
      newvars[k] = vars[i];
      var2level[k] = i;
      level2var[i] = k;
