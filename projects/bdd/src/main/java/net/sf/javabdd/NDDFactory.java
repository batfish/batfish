/**
 * NDD implementation in JavaBDD version (compatible and extendable). Based on JavaBDD package in
 * {@see <a href="https://github.com/batfish/batfish">Batfish</a>}.
 *
...
public class NDDFactory extends BDDFactory {
  ...
  /**
   * default init for NDDFactory NDD cache size will be computed based on {@link CACHE_RATIO} BDD
   * cache size will be setted as {@link bddCacheSize}
   *
  ...
  /**
   * init for batfish cuz should not modify the parameters pass to factory will use {@link
   * setVarNum} lazily to initialize nodetable and declare fields
   *
   * @param bddTableSize
   * @param bddCacheSize
  ...
  public static BDDFactory init(
      ArrayList<Integer> fields, int nddTableSize, int bddTableSize, int bddCacheSize) {
   ...
   */
  public static BDDFactory init(
      int[] fields, int nddTableSize, int bddTableSize, int bddCacheSize) {
    NDDFactory f = new NDDFactory(nddTableSize, bddTableSize, bddCacheSize);
  ...
  /**
   * user should call this method to ref(protect) NDD nodes call {@link nodeTable#ref()} in Factory
   * source file
   *
   * @param ndd
  ...
  /**
   * user should call this method to deref NDD nodes call {@link nodeTable#deref()} in Factory
   * source file
   *
   * @param ndd
   */
  ...
  /**
   * Same with {@link #getVar(int field, int index)} {@bddVarsPerField} stores {field[i]} number of
   * BDD variables for each field {@nddVarsPerField} stores {field[i]} number of NDD variables for
   * each field
   *
  ...
  public class bdd extends BDD {
    ...
    public int var() {
      ...
      for (Map.Entry<NDD, Integer> entry : this._index.edges.entrySet()) {
        ...
        if (bit == null) {
          ...
          for (; i < bitset.length(); i++) {
            if (bitset.get(i) != bit.get(i)) break;
          }
          if (bit.get(i)) {
            bit = bitset;
          }
      ...
      int offset =
          maxVariablePerField.get(this._index.field)
              - maxVariablePerField.get(this._index.field - 1);
    ...
    /**
     * Return true if this BDD tests any of the variables set in the given {@code var} BDD.
     * Equivalent to {@code !this.exist(var).equals(this)}, but doesn't create BDDs.
     *
     ...
     * </ol>
     *
     * <p>Use case: {@code rel} represents a relation (multi-valued or nondeterministic function) as
     * a constraint over unprimed and and primed variables (unprimed variables represent inputs and
     * primed variables represent outputs), {@code x} represents a set of values as a constraint
     * over unprimed variables, and {@code pair} maps the primed variables to their corresponding
     * unprimed variables. {@code x.transform(rel, pair)} returns the image of {@code x} under
     * {@code rel}, i.e. the set containing all possible results of apply {@code rel} to a value in
     * {@code x}, represented as a constraint over unprimed variables.
     *
     * @param rel
     * @param pair
     */
    ...
    /**
     * Finds one satisfying variable assignment, deterministically produced as a function of the
     * seed. Finds a BDD with exactly one variable at all levels. The new BDD implies this BDD and
     * is not false unless this BDD is false.
     *
     * @param seed
  ...
  public class NDD {
    ...
    private NDD andRec(NDD b) {
      ...
      // check the cache
      if (andCache.getEntry(this, b)) return andCache.result;
    ...
    private NDD orRec(NDD b) {
      ...
      // check the cache
      if (orCache.getEntry(this, b)) return orCache.result;

      NDD result = null;
      HashMap<NDD, Integer> newEdges = new HashMap<>();
      if (field == b.field) {
        // record edges of each node, which will 'or' with the edge pointing to FALSE of another
        // node
    ...
    private NDD notRec() {
      ...
      if (notCache.getEntry(this)) return notCache.result;
    ...
    private NDD satOne_rec() {
      ...
      int edge = 0;
      for (Map.Entry<NDD, Integer> entry : edges.entrySet()) {
        if (entry.getKey().isFalse()) continue;
        child = entry.getKey();
        edge = entry.getValue();
        break;
      }
      if (child == null) return FALSE; // TODO
    ...
    public void printRec() {
      if (isTrue()) {
        ...
        for (Map.Entry<NDD, Integer> entry : edges.entrySet()) {
          ...
          bddEngine.printDot(
              "/Users/augists/Downloads/NDD-main/" + entry.getValue(), entry.getValue());
        }
        System.out.println();
        for (NDD next : edges.keySet()) {
          next.printRec();
  ...
  public class OperationCache<T> {
     ...
     * @param entrySize The length of each entry. 3 for binary operations and 2 for unary
     *     operations.
     */
    ...
    /**
     * Insert new entry of (operand1, result) into cache. Directly overwrite the old value if there
     * exist a hash collision.
     *
    ...
    /**
     * Insert new entry of (operand1, operand2, result) into cache. Directly overwrite the old value
     * if there exist a hash collision.
     *
     * @param index The index of the entry to be inserted, which is actually a hash value.
     ...
     *
     * @param operand1 The only operand of a unary operation.
     * @return TRUE if the entry found (the result will be stored in this.result), FALSE if the
     *     entry not found (the hashValue will be stored in this.hashValue).
     */
  ...
  public int setVarNum(int num) {
    if (num > Integer.MAX_VALUE / 2) throw new BDDException();
  ...
  public NDD toNDD(int a) {
    ...
    for (HashMap<Integer, Integer> map : decomposed.values())
      for (Integer pred : map.values()) bddEngine.deref(pred);
  ...
  private void detect_boundary_point(
      int root,
      int curr,
  ...
  private int construct_decomposed_bdd(int root, int end_point, int curr) {
    ...
    if (curr == 1) {
      if (end_point == 1) return 1;
      else return 0;
    } else if (bdd_getField(root) != bdd_getField(curr)) {
      if (end_point == curr) return 1;
      else return 0;
    }
  ...
  private void get_boundary_tree(
      int a,
      HashMap<Integer, HashSet<Integer>> boundary_tree,
      ArrayList<HashSet<Integer>> boundary_points) {
    int start_level;
  ...
  private int bdd_getField(int a) {
    ...
    if (a == 1 || a == 0) return fieldNum;
    int curr = 0;
    while (curr < fieldNum) {
      if (va <= maxVariablePerField.get(curr)) break;
      curr++;
    }
    return curr;
  }
  ...
  /** NDD node count BDD node count BDD operation cache status */
