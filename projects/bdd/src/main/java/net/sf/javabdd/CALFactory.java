...
/**
 * An implementation of BDDFactory that relies on the CAL library through a native interface. You
 * can use this by calling the "CALFactory.init()" method with the desired arguments. This will
 * return you an instance of the BDDFactory class that you can use. Call "done()" on that instance
 * when you are finished.
 *
 * <p>CAL does not have much of the functionality that BuDDy has, and it has not been well-tested.
 * Furthermore, it is slower than BuDDy. Therefore, it is recommended that you use the BuDDy library
 * instead.
 *
 * <p>This class (and the CAL library) do NOT support multithreading. Furthermore, there can be only
 * one instance active at a time. You can only call "init()" again after you have called "done()" on
 * the original instance. It is not recommended to call "init()" again after calling "done()" unless
 * you are _completely_ sure that all BDD objects that reference the old factory have been freed.
 *
 ...
 * @author John Whaley
 * @version $Id: CALFactory.java,v 1.7 2005/05/21 08:47:09 joewhaley Exp $
 */
