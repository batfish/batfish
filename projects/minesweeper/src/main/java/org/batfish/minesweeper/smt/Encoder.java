...
public class Encoder {
  ...
  private Encoder(
    ...
    _modelIgp = q.getModelIgp(); //false to slice out IGP.
    _encodingId = id;
    _question = q;
    _slices = new HashMap<>();
    ...
    if (solver == null) {
      if (ENABLE_UNSAT_CORE) {
        ...
        // No parallel solving in Z3 Java API... that's ok probably wouldn't make a big difference.
        Params p = _ctx.mkParams();
        p.add("arith.nl", false);
        //p.add("parallel.enable", true);
  ...
  private void initSlices(HeaderSpace h, Graph g) {
    if (g.getIbgpNeighbors().isEmpty() /*|| !_modelIgp*/) {
      _slices.put(MAIN_SLICE_NAME, new EncoderSlice(this, h, g, "", true));
    } else {
      _slices.put(MAIN_SLICE_NAME, new EncoderSlice(this, h, g, MAIN_SLICE_NAME, true));
    }
    ...
    SortedSet<MsPair<String, Ip>> ibgpRouters = new TreeSet<>();
      ...
      for (Entry<GraphEdge, BgpActivePeerConfig> entry : g.getIbgpNeighbors().entrySet()) {
        ...
        if (!ibgpRouters.contains(pair)) {
          ...
          EncoderSlice slice = new EncoderSlice(this, hs, gNew, sliceName, _modelIgp);

          // if IGP is not modelled then do not add this slice to the map of slices
          // so that no constraints are generated for it.
          if (_modelIgp) {
            _slices.put(sliceName, slice);
          }

          PropertyAdder pa = new PropertyAdder(slice);
          Map<String, BoolExpr> reachVars;
          //TODO: check if this is a sane encoding\
          // and we are adding the right constraints and not more than that
        // If IGP is modelled then add the proper reachability variables.
        if (_modelIgp) {
          reachVars = pa.instrumentReachability(router);
        }
        else {
          // if Igp is not modelled, add trivial reachability variables.
          reachVars = pa.trivialReachability(router);
        }
