...
class AbstractionBuilder {
  ...
  private NetworkSlice computeAbstraction() {
   ...
   /* for (String device : _destinations) {
    ...
    // Make it so that origination points initially represent a single abstract node (they may be further split).
    _abstractGroups.split(_destinations);
    ...
    do {
      ...
      for (Set<String> newPartition : todo) {
        ...
        Consumer<Set<String>> c = (sets) -> sets.forEach(s -> System.out.println(s));
        _abstractGroups.partitions().forEach(c.andThen(sets -> System.out.println("-----")));
        System.out.println("\nSplitting: ");
        newPartition.forEach(s -> System.out.println(s));
    ...
    Tuple<Graph, Tuple<Map<GraphEdge, Integer>, AbstractionMap>> abstractNetwork = createAbstractNetwork();
    Graph abstractGraph = abstractNetwork.getFirst();
    AbstractionMap abstractionMap = abstractNetwork.getSecond().getSecond();
    Map<GraphEdge, Integer> multiplicities = abstractNetwork.getSecond().getFirst();
     System.out.println("Num configs: " + abstractGraph.getConfigurations().size());
    Abstraction a = new Abstraction(abstractGraph, abstractionMap, multiplicities);
  ...
  private Map<Integer, Set<String>> pickCanonicalRouters() {
    ...
    if (_destinations.isEmpty()) {
      ...
      for (String d : _destinations) {
        if (!chosen.containsKey(_abstractGroups.getHandle(d))) {
          stack.push(d);
          Set<String> dest = new HashSet<>();
          dest.add(d);
  ...
  private Configuration createAbstractConfig(Set<String> abstractRouters, Configuration conf) {
    ...
    for (GraphEdge ge : edges) {
      ...
      if (leavesNetwork
        ...
        BgpActivePeerConfig m = _graph.getIbgpNeighbors().get(ge);
        if (n != null) {
          bgpPeerConfigs.add(n);
        }
        // adding iBGP neighbors too.
        if (m!= null) {
          bgpPeerConfigs.add(m);
        }
      }
    }
  ...
  /* Computes the minimum number of concrete edges that correspond to an abstract edge */
  private Map<GraphEdge, Integer> computeMultiplicities(Graph absGraph) {

    /*System.out.println("PolMap:");
    _polMap.forEach((router, id, set) ->
            set.forEach(edgePolicy -> System.out.println("Router: " + router + "with id: " + id + "\n" + edgePolicy.toString())));

    System.out.println("ImportPol");
    _importPol.forEach((ge, ip) -> System.out.println(ge.toString() + " : " + ip.toString()));*/


    Map<GraphEdge, Integer> multiplicities = new HashMap<>();
    for (GraphEdge absE : absGraph.getAllEdges()) {
      String startAbsRouter = absE.getRouter();
      String endAbsRouter = absE.getPeer();

      // Concrete routers that correspond to the abstract ones
      Set<String> startConcRouters = _abstractGroups.getPartition(_abstractGroups.getHandle(startAbsRouter));
      if (endAbsRouter != null) {
        Set<String> endConcRouters =
            _abstractGroups.getPartition(_abstractGroups.getHandle(endAbsRouter));

        // Need to find edge with same policy
        InterfacePolicy absExport = _exportPol.get(absE);
        InterfacePolicy absImport = _importPol.get(absE);

        //System.out.println("AbsEdge: " + absE.toString());
        //System.out.println("startConcRouters: ");
        //startConcRouters.forEach(s -> System.out.println(s));
        Integer curNodeMult;
        // For every concrete router that corresponds to the start of an edge
        for (String startC : startConcRouters) {
          List<GraphEdge> concEs = _graph.getEdgeMap().get(startC);
          // For every edge from that concrete router
          curNodeMult = 0;
          for (GraphEdge concE : concEs) {
            InterfacePolicy concExport = _exportPol.get(concE);
            InterfacePolicy concImport = _importPol.get(concE);
            String endConcRouter = concE.getPeer();

            //TODO: Case endConcRouter == null?
            if (endConcRouter != null) {
              // If that edge ends at a concrete router belonging to the end abstract node.
              if (endConcRouters.contains(endConcRouter)) {
            //    System.out.println("ConcE: " + concE.toString());
                if ((concExport != null && absExport != null && concExport.equals(absExport))
                    || concExport == null && absExport == null) {
                  if ((concImport != null && absImport != null && concImport.equals(absImport))
                      || concImport == null && absImport == null) {
                    curNodeMult++;
                  }
                }
              }
          ...
          Integer mult = multiplicities.get(absE);
          if (mult == null || curNodeMult < mult) {
            multiplicities.put(absE, curNodeMult);
          }
        }
      }
    }
   // multiplicities.forEach((ge, i) -> System.out.println(ge.toString() + ": " + i.toString()));
   ...
   */
  private Tuple<Graph, Tuple<Map<GraphEdge, Integer>, AbstractionMap>> createAbstractNetwork() {
    ...
    Map<GraphEdge, Integer> multiplicities = computeMultiplicities(abstractGraph);
    Tuple<Map<GraphEdge, Integer>, AbstractionMap> multMap = new Tuple<>(multiplicities,map);
    return new Tuple<>(abstractGraph, multMap);
  }
}
