...
class EncoderSlice {
   ...
   */
  EncoderSlice(Encoder enc, HeaderSpace h, Graph graph, String sliceName, boolean modelIgp) {
    _encoder = enc;
    ...
    if (modelIgp) {
      ...
      enc.getAllVariables()
          .put(_symbolicPacket.getTcpAck().toString(), _symbolicPacket.getTcpAck());
      enc.getAllVariables()
          .put(_symbolicPacket.getTcpCwr().toString(), _symbolicPacket.getTcpCwr());
      enc.getAllVariables()
          .put(_symbolicPacket.getTcpEce().toString(), _symbolicPacket.getTcpEce());
      enc.getAllVariables()
          .put(_symbolicPacket.getTcpFin().toString(), _symbolicPacket.getTcpFin());
      enc.getAllVariables()
          .put(_symbolicPacket.getTcpPsh().toString(), _symbolicPacket.getTcpPsh());
      enc.getAllVariables()
          .put(_symbolicPacket.getTcpRst().toString(), _symbolicPacket.getTcpRst());
      enc.getAllVariables()
          .put(_symbolicPacket.getTcpSyn().toString(), _symbolicPacket.getTcpSyn());
      enc.getAllVariables()
          .put(_symbolicPacket.getTcpUrg().toString(), _symbolicPacket.getTcpUrg());
      enc.getAllVariables()
   ...
   * NOTE: Does not compare clusterSet, but only clusterLen.
   */
  public BoolExpr equal(
    ...
    ArithExpr defaultClusterLen = mkInt(defaultClusterLen());
    ...
    equalClusterLen = equalHelper(best.getClusterLen(), vars.getClusterLen(), defaultClusterLen);
  ...
  private BoolExpr greaterOrEqual(
    ...
    BoolExpr betterClusterLen = geBetterHelper(best.getClusterLen(), vars.getClusterLen(), defaultClusterLen, true);
    BoolExpr equalClusterLen = geEqualHelper(best.getClusterLen(), vars.getClusterLen(), defaultClusterLen);
    ...
    bgpRankingBetter.put(BgpDecisionVariable.CLUSTERLEN, betterClusterLen);
    bgpRankingEq.put(BgpDecisionVariable.CLUSTERLEN, equalClusterLen);
  ...
  private void addBestPerProtocolConstraints() {
    //System.out.println("Working on slice: " + _sliceName);
    for (Entry<String, Configuration> entry : getGraph().getConfigurations().entrySet()) {
      String router = entry.getKey();
      Configuration conf = entry.getValue();

      //System.out.println("  on router: " + router);
      for (Protocol proto : getProtocols().get(router)) {
        ...
        //  System.out.println("Edge: " + e.getEdge().toString() + " for router " + router);
          SymbolicRoute vars = correctVars(e);

          //NOTE: actually the Or on the premise seems right, because each permit constraint is included in
          // each conjunction in the conclusion.
        ...
        //System.out.println("Best for slice:" + _sliceName + " router " + router + " and prot " + proto.name());
        //System.out.println(mkImplies(somePermitted, acc).toString());
      }
    }
  }
    ...
    for (Entry<String, List<GraphEdge>> entry : getGraph().getEdgeMap().entrySet()) {
      ...
      for (GraphEdge ge : edges) {
        ...
        if (!ge.isAbstract()) {
          ...
          // out the current edge ge, then we use ge.

          // For the new implementation where cluster-len is used for loop prevention,
          // and everything is connected I am going to handle this as normal eBGP.
          // TODO: Not sure this is the right way to avoid modelling iBGP, check.

          /* Ok this is a huge hack, but bear with me.
             It leverages the idea that we have both iBGP and physical connectivity between the two devices.
             This should be the model in the case where we don't model igp.
             Need to do something different (something similar to what was happening before) in case where
             we are modelling IGP, but it may not be of interest right now.
             The whole iBGP model of MineSweeper requires a revision which we will likely not get to do for now.
             TODO: Do the revision of iBGP!
           */
          for (GraphEdge ge2 : getGraph().getEdgeMap().get(router)) {
            if (ge2.isAbstract() && isMainSlice()) {
                if (ge2.getRouter().equals(ge.getRouter()) && (ge2.getPeer().equals(ge.getPeer()))) {
                  // Ensure that underlying edge is not failed. This is important to compute the right forwarding
                  // model, not so much for reachability because we handle that elsewhere (trivialReachability).
                  //BoolExpr isNotFailed =
                    //      mkEq(_encoder.mkInt(0), _encoder.getSymbolicFailures().getFailedVariable(ge));
                  fwd = mkOr(fwd,getSymbolicDecisions().getControlForwarding().get(router, ge2));
                          //mkAnd(getSymbolicDecisions().getControlForwarding().get(router, ge2), isNotFailed));
                }
            }
          }
          ...
          /*for (GraphEdge ge2 : getGraph().getEdgeMap().get(router)) {
            if (ge2.isAbstract()) {
              ...
              if (st == Graph.BgpSendType.TO_RR) {
                ...
                if (isMainSlice()) {
                  for (Entry<String, Integer> entry2 : getGraph().getOriginatorId().entrySet()) {
                    ...
                    if (_encoder.getModelIgp()) {
                      ...
                      acc = mkOr(acc, record.getClientId().checkIfValue(id));
                    }
                  }
                }
                ...
                // adjust for iBGP in main slice
                if (isMainSlice()) {
                  BoolExpr outEdge = mkTrue();
                  if (_encoder.getModelIgp()) {
                    EncoderSlice s = _encoder.getSlice(ge2.getPeer());
                    if (otherSliceHasEdge(s, router, ge)) {
                      outEdge = s.getSymbolicDecisions().getDataForwarding().get(router, ge);
                    }
                  }
  ...
  private void addImportConstraint(
    ...
    if (vars.getIsUsed()) {
      ...
      if (proto.isConnected()) {
        ...
        BoolExpr clusterLen = safeEq(vars.getClusterLen(), mkInt(0));
        BoolExpr values = mkAnd(per, len, ad, lp, met, clusterLen);
      ...
      if (proto.isOspf() || proto.isBgp()) {
        ...
        if (varsOther != null) {
          ...
          TransferSSA f =
              new TransferSSA(this, conf, varsOther, vars, proto, statements, cost, 0, ge, false);
          importFunction = f.compute();

          BoolExpr acc = mkIf(usable, importFunction, val);

          //System.out.println("usable constraint on " + ge.toString() + " " + acc);
    ...
    if (!_optimizations.getSliceCanKeepSingleExportVar().get(router).get(proto) || !usedExport) {
      ...
      if (proto.isOspf() || proto.isBgp()) {
        ...
        if (isInternalExport && proto.isBgp() && isNonClientEdge) {
          if (isClientEdge) {
            // Router is a reflector, sending to a client.
            // TODO: Use a null value for cost, to denote that we should not increase path because it is iBGP.
            cost = 0;
            clusterCost = 0;
          }
          else {
            // Lookup if we learned from iBGP, and if so, don't export the route
            // NOTE: We have changed the way iBGP loops are handled. We use the clusterLen
            // to avoid getting into loops, similar as to how ASLen is used.
            // This is the case where we are sending a route to an RR.
            ...
            if (other.getBgpInternal() != null) {
              ...
              cost = 0; // add 0 to the AS path metric
              // but add 1 to the clusterCost (this is a slight-over approximation, first iBGP node should not add 1
              clusterCost = 1;
            }
            else {
              // If it's a route we learned from eBGP, reset clusterCost
              clusterCost = null;
            }
          }
        }
        ...
        TransferSSA f =
            new TransferSSA(this, conf, varsOther, vars, proto, statements, cost, clusterCost, ge, true);
        acc = f.compute();
        ...
        if (ospfRedistribVars != null) {
          ...
          f =
              new TransferSSA(
                  this, conf, overallBest, ospfRedistribVars, proto, statements, cost, clusterCost, ge, true);
        ...
        for (Prefix p : originations) {
          ...
          if (proto.isOspf()) {
            ...
            BoolExpr values =
                mkAnd(per, lp, ad, met, clusterLen, med, len, type, area, internal, igpMet, comms);
  ...
  private void addUnusedDefaultValueConstraints() {
    for (SymbolicRoute vars : getAllSymbolicRecords()) {
      ...
      if (vars.getClusterLen() != null) {
        add(mkImplies(notPermitted, mkEq(vars.getClusterLen(), zero)));
      }
