...
public class SymbolicRoute implements IDeepCopy<SymbolicRoute> {
  ...
  // private Map<String, BoolExpr> _clusterSet;

  // Should match with the number of true variables of _clusterSet.
  private ArithExpr _clusterLen;
  ...
  SymbolicRoute(SymbolicRoute other) {
   ...
   // _clusterSet =
    //    new HashMap<>(other._clusterSet);  // TODO: same as above.
    _clusterLen = other._clusterLen;
  }
  ...
  SymbolicRoute(
    ...
    if (proto.isBest()) {
      ...
      _clusterLen = (modelIbgp ? ctx.mkIntConst(_name + "_clusterLen") : null);
    ...
    // Initialize clusterSet with routers to their respective boolean expressions.
    // TODO: For now we represent all routers, but it should only be iBGP routers within
    //  the same ASN.
    /*  if (usesBgp) {
      for (String rtr : slice.getGraph().getRouters()) {
        BoolExpr var = ctx.mkBoolConst(_name + "_clusterSet_" + rtr);
        _clusterSet.put(rtr, var);
      }
      } */
  ...
  private void addExprs(EncoderSlice enc) {
    ...
    if (_clusterLen != null) {
      all.put(_clusterLen.toString(), _clusterLen);
    }
    if (_ospfArea != null) {
  ...
  /*  for (BoolExpr var : _clusterSet.values()) {
      all.put(var.toString(), var);
    } */
  }
 ...
 /* Map<String, BoolExpr> getClusterSet() {
    return _clusterSet;
  }

  public void setClusterSet(Map<String, BoolExpr> clusterSet) {
    this._clusterSet = clusterSet;
  } */
  ...
  ArithExpr getClusterLen() {
    return _clusterLen;
  }

  public void setClusterLen(ArithExpr clusterLen) {
    this._clusterLen = clusterLen;
  }
