...
class TransferBDD {
  ...
  private TransferResult<TransferReturn, BDD> compute(BooleanExpr expr, TransferParam<BDDRoute> p) {
    ...
    if (expr instanceof MatchPrefixSet) {
      ...
      p.debug("MatchClusterLen");
      MatchClusterLen mcl = (MatchClusterLen) expr;
      BDDInteger mclInt = BDDInteger.makeFromValue(factory, 32, mcl.getClusterLen());
      BDD eq = mkBDD(mclInt.equals(p.getData().getClusterLen()));
      TransferReturn ret = new TransferReturn(p.getData(), eq);
      return fromExpr(ret);
      }
  ...
  private TransferResult<TransferReturn, BDD> compute(
    ...
    for (Statement stmt : statements) {
      ...
      if (stmt instanceof StaticStatement) {
        ...
        //TODO: This may not work for other protocols, such as OSPF but ignore for now.
       ...
       // BDD updateMed = isBGP.and(result.getReturnAssignedValue());
        ...
        BDDInteger med = ite(result.getReturnAssignedValue(), curP.getData().getMed(), newValue);
        //BDDInteger met = ite(updateMet, curP.getData().getMetric(), newValue);
        //curP.getData().setMetric(met);
        System.out.println("Setting med: " + ((LiteralLong) ie).getValue());
