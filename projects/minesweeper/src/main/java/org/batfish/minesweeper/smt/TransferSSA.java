...
class TransferSSA {
  ...
  @Nullable
  private Integer _addedClusterCost; // for iBGP clusterLen.
  ...
  TransferSSA(
    ...
    _addedClusterCost = addedClusterCost;
    _graphEdge = ge;
    _iface = ge.getStart();
    _isExport = isExport;
    _aggregates = null;
  ...
  private BoolExpr matchCommunityList(CommunityList cl, SymbolicRoute other) {
    ...
    for (CommunityListLine line : lines) {
      ...
      // BoolExpr c = other.getCommunities().get(cvar); //This is wrong, we need to actually match on the cvar.
  ...
  private TransferResult<BoolExpr, BoolExpr> compute(
    ...
    if (expr instanceof MatchPrefixSet) {
      ...
      pCur.debug("MatchTag");
      System.out.println("Warning: use of unimplemented feature MatchTag");
      return fromExpr(_enc.mkFalse());
    } else if (expr instanceof MatchClusterLen) {
      pCur.debug("MatchClusterLen");
/*      MatchClusterLen mcl = (MatchClusterLen) expr;
      BoolExpr guard = _enc.mkEq(_enc.mkInt(mcl.getClusterLen()), pCur.getData().getClusterLen());
      return fromExpr(guard); */
      return fromExpr(_enc.mkTrue());
    }
    else {
  ...
  private BoolExpr relateVariables(
    ...
    ArithExpr defaultClusterLen = _enc.mkInt(_enc.defaultClusterLen());
    ...
    //NOTE: This code doesn't exactly work with our model of iBGP, and it's not relevant to it anyway because
    // we do not model IGP.
          ...
          for (Map.Entry<String, Integer> entry : _enc.getGraph().getOriginatorId().entrySet()) {
            ...
            if (!r.equals(router)) {
              ...
              if (s != null) {
                ...
                eq = _enc.mkEq(_current.getIgpMetric(), record.getMetric());
              } else {
                eq = _enc.mkEq(_current.getIgpMetric(), defaultMet);
              }
              acc =
                      _enc.mkAnd(
    ...
    ArithExpr otherClusterLen = getOrDefault(p.getData().getClusterLen(), defaultClusterLen);
    ...
    BoolExpr clusterLen = _enc.safeEq(_current.getClusterLen(), otherClusterLen);
    ...
    BoolExpr updates =
        _enc.mkAnd(
            per, len, ad, med, lp, met, id, cid, type, area, clusterLen, comms, history, isInternal, igpMet);
  ...
  private void updateSingleValue(TransferParam<SymbolicRoute> p, String variableName, Expr expr) {
    switch (variableName) {
      case "METRIC":
        ...
        break;
      case "CLUSTERLEN":
        p.getData().setClusterLen((ArithExpr) expr);
        break;
      case "MED":
  ...
  private MsPair<Expr, Expr> joinPoint(
    ...
    if (variableName.equals("CLUSTERLEN")) {
      Expr t = (trueBranch == null ? p.getData().getClusterLen() : trueBranch);
      Expr f = (falseBranch == null ? p.getData().getClusterLen() : falseBranch);
      ArithExpr newValue = _enc.mkIf(guard, (ArithExpr) t, (ArithExpr) f);
      newValue = _enc.mkIf(r.getReturnAssignedValue(), p.getData().getClusterLen(), newValue);
      ArithExpr ret = createArithVariableWith(p, "CLUSTERLEN", newValue);
      p.getData().setClusterLen(ret);
      return new MsPair<>(ret, null);
    }
    if (variableName.equals("MED")) {
      ...
      ArithExpr ret = createArithVariableWith(p, "MED", newValue);
    ...
    for (Statement stmt : statements) {
      if (stmt instanceof StaticStatement) {
        ...
        //System.out.println("Warning: use of unimplemented feature SetNextHop");
          ...
          curP.debug("SetTag");
          System.out.println("Warning: use of unimplemented feature SetTag");
      }  else if (stmt instanceof SetWeight) {
        curP.debug("SetWeight");
        System.out.println("Warning: use of unimplemented feature SetWeight");
      } else if (stmt instanceof CallStatement) {
        String s = (_isExport ? "export" : "import");
        List<Statement> targetStmts = ((CallStatement) stmt).getCalledPolicyStatements(_conf);
        System.out.println(
                String.format(
                        "Unimplemented CallStatement %s for %s transfer function on interface %s, " +
                                "should have been inlined",
                        ((CallStatement) stmt).getCalledPolicyName(), s, _graphEdge.toString()));
      }
      else {
        String s = (_isExport ? "export" : "import");
        String msg =
  ...
  private void applyMetricUpdate(TransferParam<SymbolicRoute> p) {
    ...
    if (updateMetric) {
      ...
      if (_proto.isBgp()) {
        ...
        if (_addedClusterCost != null)
        {
          ArithExpr clusterCost = _enc.mkInt(_addedClusterCost);
          ArithExpr clusterSum = _enc.mkSum(p.getData().getClusterLen(), clusterCost);
          p.getData().setClusterLen(clusterSum);
        }
        else
        {
          p.getData().setClusterLen(_enc.mkInt(0));
        }
      } else {
        newValue = sum;
      }
