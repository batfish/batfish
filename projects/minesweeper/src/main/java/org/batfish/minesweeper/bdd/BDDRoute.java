...
public class BDDRoute implements IDeepCopy<BDDRoute> {
  ...
  private BDDInteger _clusterLen;
  ...
  public BDDRoute(Set<CommunityVar> comms) {
    ...
    int numNeeded = 32 * 6 + 6 + comms.size() + 4;
    ...
    idx += 32;
    _clusterLen = BDDInteger.makeFromIndex(factory, 32, idx, false);
    addBitNames("clusterLen", 32, idx, false);
    idx += 32;
  ...
  public BDDRoute(BDDRoute other) {
    ...
    _clusterLen = new BDDInteger(other._clusterLen);
  ...
  public BDDInteger getClusterLen() {
    return _clusterLen;
  }

  public void setClusterLen(BDDInteger clusterLen) {
  ...
  public int hashCode() {
    if (_hcode == 0) {
      ...
      result = 31 * result + (_clusterLen != null ? _clusterLen.hashCode() : 0);
  ...
  public boolean equals(Object o) {
    ...
    return Objects.equals(_metric, other._metric)
        ...
        && Objects.equals(_clusterLen, other._clusterLen)
  ...
  public void orWith(BDDRoute other) {
    ...
    BDD[] clusterLen = getClusterLen().getBitvec();
    ...
    BDD[] clusterLen2 = other.getClusterLen().getBitvec();
    ...
    for (int i = 0; i < 32; i++) {
      ...
      clusterLen[i].orWith(clusterLen2[i]);
    }
  ...
  public BDDRoute restrict(Prefix pfx) {
    ...
    BDD[] clusterLen = rec.getClusterLen().getBitvec();
    ...
    for (int i = 0; i < 32; i++) {
      ...
      clusterLen[i] = clusterLen[i].veccompose(pairing);
    }
    for (int i = 0; i < ospfMet.length; i++) {
