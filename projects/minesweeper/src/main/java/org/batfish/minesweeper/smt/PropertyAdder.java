...
class PropertyAdder {
  ...
  /*
   * Adds trivial reachability constraint to a destination router.
   */
  Map<String, BoolExpr> trivialReachability(String router) {
    Context ctx = _encoderSlice.getCtx();
    Solver solver = _encoderSlice.getSolver();
    Map<String, BoolExpr> reachableVars = new HashMap<>();
    String sliceName = _encoderSlice.getSliceName();

    Set<String> ges = new HashSet<>();
    SymbolicFailures failedVars = _encoderSlice.getEncoder().getSymbolicFailures();
    for (GraphEdge e : _encoderSlice.getGraph().getEdgeMap().get(router)) {
      String r = e.getPeer();
      if (r != null && !ges.contains(r)) {
        int id = _encoderSlice.getEncoder().getId();
        String s2 = id + "_" + sliceName + "_reachable_" + r;
        BoolExpr var = ctx.mkBoolConst(s2);
        reachableVars.put(r, var);
        _encoderSlice.getAllVariables().put(var.toString(), var);
        // We have (trivial) reachability if the link is not failed.
        solver.add(ctx.mkIff(var,ctx.mkEq(failedVars.getFailedVariable(e),ctx.mkInt(0))));
        ges.add(r);
      }
    }
    return reachableVars;
  ...
  /* Just a wrapper, taking arguments of type List instead of [] */
  private static BoolExpr mkAtLeast(Context ctx, List<BoolExpr> exprs, int k) {
    BoolExpr[] args = new BoolExpr[exprs.size()];
    // Require that at least k of the route expressions are true.
    return ctx.mkAtLeast(exprs.toArray(args), k);
  }

  // Wrapper around pbGE function,
  // k_1*e1 + ... k_n*e_n >= k
  private static BoolExpr mkPBGe(Context ctx, List<Tuple<Integer, BoolExpr>> exprs, int k) {
    BoolExpr[] args = new BoolExpr[exprs.size()];
    int[] coeffs = new int[exprs.size()];
    for (int i = 0; i < exprs.size(); i++) {
      Tuple<Integer, BoolExpr> expr = exprs.get(i);
      coeffs[i] = expr.getFirst();
      args[i] = expr.getSecond();
    }
    // Require that at least k of the route expressions are true.
    return ctx.mkPBGe(coeffs,args, k);
  }

  /*
   * Generates constraints for reachability through k or more neighbors.
   * Similar to recursiveReachability but requires k forwarding neighbors.
   */
  private BoolExpr recursiveReachabilityWidth(
          Context ctx,
          EncoderSlice slice,
          List<GraphEdge> edges,
          Map<String, ArithExpr> idVars,
          ...
          int k) {
    ArithExpr zero = ctx.mkInt(0);
    List<BoolExpr> recursiveRoutes = new ArrayList<>();
    BoolExpr largerIds = ctx.mkTrue();
    for (GraphEdge edge : edges) {
      if (!edge.isAbstract()) {
        BoolExpr fwd = _encoderSlice.getForwardsAcross().get(router, edge);
        if (edge.getPeer() != null) {
          ...
          BoolExpr peerReachable = ctx.mkGt(peerId, zero);
          BoolExpr sendToReachable = ctx.mkAnd(fwd, peerReachable);
          recursiveRoutes.add(sendToReachable);
          BoolExpr increasingId = ctx.mkImplies(sendToReachable, ctx.mkGt(id, peerId));
          largerIds = ctx.mkAnd(largerIds, increasingId);
        }
      }
    }
    // Require at least k reachable neighbors for reachability.
    BoolExpr atLeast = mkAtLeast(ctx, recursiveRoutes, k);
    return slice.mkIf(atLeast, largerIds, ctx.mkEq(id, zero));
  }
   ...
   * Similar to recursiveReachability but requires k forwarding neighbors.
   */
  private BoolExpr recursiveReachabilityWidthAbstract(
          Context ctx,
          EncoderSlice slice,
          List<GraphEdge> edges,
          Map<String, ArithExpr> idVars,
          String router,
          ArithExpr id,
          Map<GraphEdge, Integer> mult,
          int k) {
    ArithExpr zero = ctx.mkInt(0);
    List<Tuple<Integer, BoolExpr>> recursiveRoutes = new ArrayList<>();
    BoolExpr largerIds = ctx.mkTrue();
    for (GraphEdge edge : edges) {
      if (!edge.isAbstract()) {
        BoolExpr fwd = _encoderSlice.getForwardsAcross().get(router, edge);
        if (edge.getPeer() != null) {
          ...
          BoolExpr peerReachable = ctx.mkGt(peerId, zero);
          BoolExpr sendToReachable = ctx.mkAnd(fwd, peerReachable);
          recursiveRoutes.add(new Tuple<>(mult.get(edge), sendToReachable));
          BoolExpr increasingId = ctx.mkImplies(sendToReachable, ctx.mkGt(id, peerId));
          largerIds = ctx.mkAnd(largerIds, increasingId);
        }
      }
    ...
    // Require at least k reachable neighbors for reachability.
    BoolExpr atLeast = mkPBGe(ctx, recursiveRoutes, k);
    return slice.mkIf(atLeast, largerIds, ctx.mkEq(id, zero));
  }
  ...
  /*
   * Add k-reachability information to the network for destination interfaces.
   */
  Map<String, BoolExpr> instrumentReachabilityWidth(Set<GraphEdge> ges, Map<GraphEdge,Integer> multiplicities, int k) {
    Context ctx = _encoderSlice.getCtx();
    Solver solver = _encoderSlice.getSolver();
    EncoderSlice slice = _encoderSlice;
    Map<String, BoolExpr> reachableVars = new HashMap<>();
    ...
    // Iterate over all routers and their links
    for (Entry<String, List<GraphEdge>> entry : g.getEdgeMap().entrySet()) {
      String router = entry.getKey();
      List<GraphEdge> edges = entry.getValue();
      ArithExpr id = idVars.get(router);
      // Add the base case, reachable if we forward to a directly connected interface
      List<BoolExpr> externalRoutes = new ArrayList<>();
      BoolExpr isHost = ctx.mkFalse();
      BoolExpr isAbsorbed = ctx.mkFalse();
      SymbolicRoute r = _encoderSlice.getBestNeighborPerProtocol(router, Protocol.CONNECTED);

      for (GraphEdge ge : edges) {
        if (!ge.isAbstract() && ges.contains(ge)) {
          // If a host, consider reachable, do not count in this case
          if (g.isHost(router)) {
            isHost = ctx.mkTrue();
            break;
          }
          // Reachable if we leave the network, but we need at least k exit points.
          if (ge.getPeer() == null) {
            BoolExpr fwdIface = _encoderSlice.getForwardsAcross().get(ge.getRouter(), ge);
            assert (fwdIface != null);
            externalRoutes.add(fwdIface);
          }
          // Also reachable if connected route and we use it despite not forwarding
          if (r != null) {
            BitVecExpr dstIp = _encoderSlice.getSymbolicPacket().getDstIp();
      ...
      BoolExpr recursive;
      // Add the recursive case, where it is reachable through a neighbor
      if (_encoderSlice.getEncoder().getQuestion().getUseAbstraction()) {
        assert (multiplicities != null);
        recursive = recursiveReachabilityWidthAbstract(ctx, slice, edges, idVars, router, id, multiplicities, k);
      } else {
        recursive = recursiveReachabilityWidth(ctx, slice, edges, idVars, router, id, k);
        }
      BoolExpr hasExternal = mkAtLeast(ctx, externalRoutes, k);
      // Reachable if it is a host, it is absorbed, or has k-external neighbors.
      // TODO: discuss if that's the intent.
      BoolExpr guard = ctx.mkOr(isHost, isAbsorbed, hasExternal);
      BoolExpr cond = slice.mkIf(guard, ctx.mkEq(id, ctx.mkInt(1)), recursive);
      solver.add(cond);
    }
    return reachableVars;
