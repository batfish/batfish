...
public class SyntacticDifference implements Comparable<SyntacticDifference> {
   ...
   *     policy in the current snapshot, the policy in the reference snapshot, and the routing
   ...
   *     lead to two seemingly equal policies being considered different, but this can only cause a
   *     performance issue by running CRP more than necessary. The results are anyway deduplicated.
   */
  @Override
  public int compareTo(SyntacticDifference o) {

    if (routingPolicySyntaxEq(this.getCurrentPolicy(), o.getCurrentPolicy())
