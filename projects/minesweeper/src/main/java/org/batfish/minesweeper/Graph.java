...
public class Graph {
  ...
  private Map<String, String> _routeReflectorParent; //TODO: change value to Set<String>
    ...
    for (Entry<String, Set<NodeInterfacePair>> entry : routerIfaceMap.entrySet()) {
      ...
      for (NodeInterfacePair nip : nips) {
        ...
        if (hasNoOtherEnd) {
          ...
          if (isLoopback(ge) || isHost(router)) {
            graphEdges.add(ge);
          }
          //graphEdges.add(ge);
        }
    ...
    for (Entry<String, Configuration> entry : _configurations.entrySet()) {
      ...
      if (conf.getDefaultVrf().getBgpProcess() != null) {
        ...
        for (BgpActivePeerConfig neighbor : bgp.getActiveNeighbors().values()) {
          // create a (two) list(s) to ip over which the connection is established and the bgp process.
  ...
  static Table2<String, String, BgpActivePeerConfig> generateIbgpNeighbors(
        ...
        for (Entry<String, Set<Ip>> ipEntry : ips.entrySet()) {
          ...
          for (Ip candidateLocalIp : candidateIps) {
            // Check that it's not a self-edge and candidate's IP matches remote prefix
              //System.out.println("candidate local ip:" + candidateLocalIp.toString()) ;
              //System.out.println("Remote prefix:" + remotePrefix.toString());
    ...
    // neighbors.forEach((w,r,config) -> System.out.println(w + ", " + r + " : " + config.toString()));
    return neighbors;
  }
