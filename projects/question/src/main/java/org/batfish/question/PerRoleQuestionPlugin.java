...
public class PerRoleQuestionPlugin extends QuestionPlugin {
   ...
   public static class PerRoleAnswerer extends Answerer {
      ...
      public PerRoleAnswerElement answer() {
         ...
         List<String> nodes = CommonUtil.getMatchingStrings(question.getNodeRegex(),
         ...
         for(String node : nodes) {
            ...
            for (String role : roles) {
               ...
               if(roleMembers == null) {
                  roleMembers = new TreeSet<String>();
    ...
    * to end hosts within the network.  Roles can be useful for improving the
    * precision and utility of several other questions.  For example, it may only
    ...
    *
    * @param question
    *           The question to ask on each role.  This parameter is mandatory.
    * @param roles
    *           List of the role names to include in the answer.  Default is to
    *           use all role names.
    * @param nodeRegex
   ...
   @Override
   protected PerRoleAnswerer createAnswerer(Question question,
         IBatfish batfish) {
