...
public class BgpSessionStatusAnswerer extends Answerer {
  ...
  public Multiset<BgpSessionInfo> rawAnswer(BgpSessionStatusQuestion question) {
    ...
    Network<BgpNeighbor, BgpSession> configuredBgpTopology = CommonUtil.initBgpTopology(configurations,
        ipOwners,
        true);

    Network<BgpNeighbor, BgpSession> establishedBgpTopology = question.getIncludeEstablishedCount()
        ? CommonUtil.initBgpTopology(configurations,
        ipOwners,
        false,
        true,
    ...
    for (BgpNeighbor bgpNeighbor : configuredBgpTopology.nodes()) {
      ...
      bgpSessionInfo._sessionType = ebgp
      ...
      if (bgpNeighbor.getDynamic()) {
        ...
        bgpSessionInfo._onLoopback = CommonUtil.isActiveLoopbackIp(localIp, configurations.get(hostname));
        ...
        if (!allInterfaceIps.contains(localIp)) {
          ...
          if (configuredBgpTopology.adjacentNodes(bgpNeighbor).isEmpty()) {
            ...
            BgpNeighbor remoteNeighbor = configuredBgpTopology.adjacentNodes(bgpNeighbor)
                .iterator()
                .next();
