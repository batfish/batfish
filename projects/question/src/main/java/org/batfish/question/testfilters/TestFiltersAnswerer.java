...
import org.batfish.datamodel.PhcIpResolvingUtil;
...
/** {@link Answerer} for {@link TestFiltersQuestion} */
public class TestFiltersAnswerer extends Answerer {
  ...
  private SortedSet<Flow> getFlows(Configuration c, ImmutableSet.Builder<String> allProblems) {
    ...
    HeaderSpace hs =
        PacketHeaderConstraintsUtil.toHeaderSpace(
            question.getHeaders(),
            PhcIpResolvingUtil.resolveSrcIpSpaceOrUniverse(
                question.getHeaders().getSrcIps(), srcLocations, _batfish.specifierContext()),
            PhcIpResolvingUtil.resolveDstIpSpaceOrUniverse(
                question.getHeaders().getSrcIps(), _batfish.specifierContext()));

    Optional<Flow.Builder> flowBuilderOption =
        new HeaderSpaceToFlow(c.getIpSpaces())
            .getRepresentativeFlow(hs, FlowPreference.APPLICATION);
    ...
    if (srcLocations.isEmpty() && question.getStartLocation() == null) {
      try {
        Flow.Builder flowBuilder = flowBuilderOption.get();
