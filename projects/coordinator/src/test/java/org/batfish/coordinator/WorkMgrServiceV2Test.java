...
public class WorkMgrServiceV2Test extends JerseyTest {
  ...
  public void getContainers() throws Exception {
    Response response = target().path("/v2/containers").request().get();
  ...
  public void redirectContainer() throws Exception {
    Response response =
        target().property(FOLLOW_REDIRECTS, false).path("/v2/container").request().get();
  ...
  public void getContainer() throws Exception {
    ...
    Container expected = Container.makeContainer(containerName, new TreeSet<>());
