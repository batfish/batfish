...
start() {
   ...
   su -c "bash -c '/usr/bin/java -DbatfishCoordinatorPropertiesPath=$COORDINATOR_PROPERTIES -cp \$(cat $COORDINATOR_CLASSPATH):$COORDINATOR_JAR $COORDINATOR_MAIN_CLASS -logfile $COORDINATOR_LOG -containerslocation $BATFISH_HOME &>> $COORDINATOR_JAVA_LOG & echo \\\$! > $COORDINATOR_PID_FILE'" batfish
...
package() {
   ...
   BATFISH_JAR_SRC_NAME=batfish-bundle-${BATFISH_VERSION}.jar
   BATFISH_JAR_SRC=$BATFISH_PATH/projects/batfish/target/$BATFISH_JAR_SRC_NAME
   ...
   CLIENT_JAR_SRC_NAME=batfish-client-bundle-${BATFISH_VERSION}.jar
   CLIENT_JAR_SRC=$BATFISH_PATH/projects/batfish-client/target/$CLIENT_JAR_SRC_NAME
   ...
   COORDINATOR_JAR_SRC_NAME=coordinator-bundle-${BATFISH_VERSION}.jar
   COORDINATOR_JAR_SRC=$BATFISH_PATH/projects/coordinator/target/${COORDINATOR_JAR_SRC_NAME}
