...
batfish_analyze() {
   ...
   $BATFISH_CONFIRM && { batfish_find_multipath_inconsistent_packet_constraints $REACH_PATH $QUERY_PATH $MPI_QUERY_BASE_PATH $NODE_SET_PATH "$MACHINES" "$NUM_MACHINES" || return 1 ; }

   echo "Generate multipath-inconsistency concretizer queries"
   $BATFISH_CONFIRM && { batfish_generate_multipath_inconsistency_concretizer_queries $MPI_QUERY_BASE_PATH $NODE_SET_PATH "$MACHINES" "$NUM_MACHINES"|| return 1 ; }
...
batfish_find_multipath_inconsistent_packet_constraints() {
   ...
   mkdir -p $QUERY_PATH
   cd $QUERY_PATH
   batfish -mpi -mpipath $QUERY_BASE_PATH -nodes $NODE_SET_PATH || return 1
   ...
   sort -R $NODE_SET_TEXT_PATH | parallel --eta --joblog joblog --tag --halt 2 $SERVER_OPTS batfish_find_multipath_inconsistent_packet_constraints_helper {} $REACH_PATH $QUERY_BASE_PATH
