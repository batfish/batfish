...
class DD:
    ...
    def report_progress(self, c):
        if self.verbose and len(c) != self.__last_reported_length:
            logger.info("%d deltas left", len(c))
    ...
    def _dd(self, c, n):
        ...
        while 1:
            tc = self.test(c)
            assert (
                tc == self.FAIL or tc == self.UNRESOLVED
            ), f"Result should not be {tc}"
            ...
            if self.verbose:
                split_sizes = list(map(len, cs))
                logger.info("(run #%d): trying splits %s", run, split_sizes)
    ...
    def _dddiff(self, c1, c2, n):
        ...
        while 1:
            ...
            if self.verbose:
                logger.info(
                    "(run #%d): trying %s", run, "+".join(str(len(c) for c in cs))
                )
            ...
            if progress:
                ...
                self.report_progress(self.__listminus(next_c2, next_c1))
            else:
                if n >= len(c):
                    # No further minimizing
                    if self.verbose:
...
if __name__ == "__main__":
    ...
    (c, c1, c2) = mydd.dd(test_list)  # Invoke DD
