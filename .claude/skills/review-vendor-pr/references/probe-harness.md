...
**Escalation path, not the default.** SKILL.md is read-only: it traces by reading
both revisions with `git show`/`git grep`, and never checks out or builds a PR.
Use this file only after the user has approved execution for a specific hypothesis
(SKILL.md step 5b).

Prefer the safe form: run *your own* scratch config against the **merge-base**
code. That answers "what does the current parser do with this input?" while
executing only already-reviewed in-tree code. Running the PR's own code is a
further step — it executes a stranger's build rules and tests, so confirm that is
what the user approved, and read the diff's `BUILD.bazel`/`*.bzl`/`tools/` hunks
first.

How to parse a hand-built config on demand and compare two revisions.
...
Before `gh pr checkout`, record the branch (`git rev-parse --abbrev-ref HEAD`) and
confirm the tree is clean. The sequence below assumes the only modification to the
test class is your probe — if it already had uncommitted changes, use a separate
worktree instead of stashing.
