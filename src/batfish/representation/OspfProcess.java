...
public class OspfProcess {
   ...
   public String getIFString(int indentLevel) {
	   ...
	   if (_routerId != null)
		   retString += "\n" + Util.getIndentString(indentLevel+1) + "RouterId " + _routerId;
	   
	   if (_referenceBandwidth != null)
		   retString += "\n" + Util.getIndentString(indentLevel+1) + "ReferenceBandwidth " + _referenceBandwidth;
      ...
      for (PolicyMap lhs: _outboundPolicyMaps){
         ...
         for (PolicyMap rhs: ospf._outboundPolicyMaps){        
            if (lhs.getMapName().equals(rhs.getMapName())){
               res = lhs.sameParseTree(rhs, "Ospf:OutboundPoliMap "+prefix);
               found = true;
               if(res == false){
                  finalRes = res;
               }
